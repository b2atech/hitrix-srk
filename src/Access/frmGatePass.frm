...
Begin VB.Form frmGatepass 
   ...
   Begin VB.Frame FrameGrid 
      ...
      Begin MSFlexGridLib.MSFlexGrid msGrid 
         ...
         EndProperty
      End
      Begin VB.CommandButton cmdExitFrameGrid 
         Caption         =   "&Close"
         BeginProperty Font 
            Name            =   "Times New Roman"
            Size            =   12
...
'--- Save Grid Records
CheckForNumberGrid
'--- Update tblBags (Gate Pass Vno,Vtype)
gCn.Execute "update tblBags set GPNo=0 ,GpType=Null where GpNo=" & txt(0) & " and GPType='GP'"
With msGrid
    i = 1
    Do While i < .Rows
       If .TextMatrix(i, 0) <> "" Then '- BagNo
            '-- SNo
            SNo = CLng(.TextMatrix(i, 3)) '--SNo
            gCn.Execute "update tblBags set GpNo=" & txt(0) & " ,GPType='GP' where inwType='" & GetInwardType & "' and InwNo=" & txttmpInNo & "  and SNo=" & SNo
        End If
        i = i + 1
     Loop
 End With
...
If (CDbl(txt(6)) = 0) Or txt(5) <> oldQty Then
    MillCode = GProcGetColumnValue("TblMastAccount", "AcName", txttempAL(5), "S", "AcCode", "N")
    Wt = Qty * CDbl(GProcGetColumnValue("tblMastItem", "ItName", txttemp(4), "S", "ItStdpack", "N", " ItMillCode= " & MillCode))
    txt(6) = Wt
End If
End Sub
...
Case 0, 1, 4:
    ...
    Do While GpNo <= txtno(1)
        X = "Select * From tblAddLess where  vNo=" & GpNo & " and VType='GP' and VYear=" & gCYear
        If GProcRstOpen(RsGp, X, "R") > 0 Then
            With CryReport
                ...
                CryReport.Formulas(8) = "wCompGstin = '" & gCGSTIN & "'"
                    ...
                    End If
                   If Dir("D:\HiReports\rptGatePassPrint.rpt") <> "" Then
                        .ReportFileName = "D:\HiReports\rptGatePassPrint.rpt"
                    Else
...
'       Dim RsTmp As Recordset
       ...
       Loop
       X = Left(X, Len(X) - 1) + ")"
       LrString = "Select tblAddLess.vNo,tblAddLess.Vdt,tblMastAccount.AcName,tblMastItem.Itname,tblMastItem.ItTicket,VSubBag,VSubWt,VSubRt,VSubRtPer,DateAdd('d', tblAddLess.AdGpNo, tblAddLess.Vdt),tblMastAccount_Br.AcName,tblMastAccount.AcFaxNo,tblMastAccount_Br.AcFaxNo,tblAddLess.AdGpNo,tblMastItem.ItType " _
                   & "From tblAddLess,tblMastAccount,tblVouSub,tblMastItem,tblMastAccount tblMastAccount_Br where tblAddLess.AdAcCrCode = tblMastAccount.AcCode and tblAddLess.Vno = tblVouSub.Vno and tblAddLess.Vtype = tblVouSub.Vtype and tblAddLess.Vyear = tblVouSub.Vyear and " _
     ...
     Do While Not RsSA.EOF
        ...
        SmsString = "GP.No-" + CStr(RsSA.Fields(0)) + " Dt-" + Trim(CStr(Format(RsSA.Fields(1), "dd/mm/yy"))) & vbLf
...
X = "select BagNo,cone,Wt,Sno from tblBags where " _
& " InwType='" & GetInwardType & "' and InwNo=" & txttmpInNo & " and GpNo=" & txt(0) & " and GpType='GP' order by SNo"
Set rstblSub = New Recordset
RCount = GProcRstOpen(rstblSub, X, "R")
With msGrid
    .Clear
    .FormatString = "<Bag No.                 |>Cone       |>Weight       |SNo"
    .ColWidth(3) = 0 '-SNo
    If CLng(RCount) > CLng(txt(5)) Then '--No Of Rows = No. Of Bags
        .Rows = RCount + 1
    Else
        .Rows = txt(5) + 1
    End If
...
'On Error GoTo nobagno
    For RowIndex = 1 To .Rows - 1
        For i = 0 To rstblSub.Fields.Count - 1
            Select Case i
                Case 0 '-- Bag  No
                    '-Bag No="" and Bags row >GP Bag delete row
                    If RowIndex > 0 And Trim(.TextMatrix(RowIndex, i)) = "" And CLng(.Rows - 1) > CLng(txt(5)) Then
                        .RemoveItem (RowIndex)
                        GoTo Validate
                    End If
           ...
           End Select
        Next i
        '-- duplicate SNo InBagNo

        
        BagSNo = .TextMatrix(RowIndex, 3) '-Sno
        For i = 1 To .Rows - 1
            If i <> RowIndex And .TextMatrix(i, 3) = BagSNo Then
                MsgBox "Duplicate Selection of BagNo.", vbCritical + vbOKOnly, Me.Caption
                .Col = 0
                FrameGrid.Visible = True
                msGrid.SetFocus
                Exit Function
    ...
    If (CLng(.Rows - 1) <> CLng(txt(5))) And .Rows > 1 Then
        MsgBox "Check Bag Detail.Bags " & CLng(.Rows - 1) & " not Equal to Gate Pass Bags " & CLng(txt(5)), vbCritical + vbOKOnly, Me.Caption
        FrameGrid.Visible = True
        msGrid.SetFocus
        Exit Function
    End If
...
'-- Inward Cartton Bags
Private Sub GetInwardCartoonBags()
Dim j As Integer
Dim X As String
X = "select BagNo,cone,Wt,Sno from tblBags where " _
& " InwType='" & GetInwardType & "' and InwNo=" & txttmpInNo & " and (GpNo=" & txt(0) & " or GpNo=0) and (GpType='GP' or GpType is Null ) "

gClsSearch.SearchMultiSelField X, "", Array("Bag No", "Cone", "Wt", "SNo"), Array(1500, 1000, 1500, 0), "", "", "Sno", 0, 0, , True
With msGrid
    If gClsSearch.SearchMultiSelRetCol(0, 0) <> "" Then
        ...
        For i = 0 To UBound(gClsSearch.SearchMultiSelRetCol)
             If .Rows - 1 < j Then Exit For
            ' Else
            '    .Rows = .Rows + 1
            ' End If
            .TextMatrix(j, 0) = gClsSearch.SearchMultiSelRetCol(i, 0) '--BagNo
            .TextMatrix(j, 1) = gClsSearch.SearchMultiSelRetCol(i, 1) '--cone
            .TextMatrix(j, 2) = gClsSearch.SearchMultiSelRetCol(i, 2) '--Wt
            .TextMatrix(j, 3) = gClsSearch.SearchMultiSelRetCol(i, 3) '--Sno
             j = j + 1
        Next i
        .Col = 0
End If
