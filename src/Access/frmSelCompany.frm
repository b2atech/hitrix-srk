...
'    mTempYr = ListView1.SelectedItem.SubItems(1)
    gCCode = ListView1.SelectedItem.SubItems(1)
    gMyConn = "gCn"
        '& " " & mTempYr
    Dim rst1 As Recordset
    If GProcRstOpen(rst1, "Select * from tblCompanyYear order by compcode,compyear desc", "R", gSetupCn) > 0 Then
        rst1.MoveFirst
...
If gUserName = "om" Then
   k = GProcRstOpen(rstData, "Select CName,CCode,CGstin From tblMastCompany  where CFirmMi = 'M'   Order by CName ", "R", gSetupCn)
'   k = GProcRstOpen(rstData, "Select CName,CCode From tblMastCompany  Order by CName where CFirmMi = 'M' ", "R", gSetupCn)
Else
   k = GProcRstOpen(rstData, "Select CName,CCode,CGstin From tblMastCompany  where CIsHide = 0   Order by CName ", "R", gSetupCn)
End If
...
Do Until rstData.EOF
    ...
    mItem.text = rstData!Cname & vbNullString
   ' mItem.SubItems(1) = rstData.Fields(1) & " - " & rstData.Fields(2)
    mItem.SubItems(1) = rstData!CCode & vbNullString
    rstData.MoveNext
Loop
End Sub
