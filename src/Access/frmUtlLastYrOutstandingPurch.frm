...
Begin VB.Form frmUtlLastYrOutstandingPurch 
   BackColor       =   &H00C2E3FC&
   BorderStyle     =   0  'None
   Caption         =   " "
   ClientHeight    =   6300
   ...
   FillColor       =   &H000040C0&
   FillStyle       =   0  'Solid
   Icon            =   "frmUtlLastYrOutstandingPurch.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin VB.ComboBox cbo 
      ...
      Height          =   360
      Index           =   1
      ItemData        =   "frmUtlLastYrOutstandingPurch.frx":058A
      Left            =   1950
      List            =   "frmUtlLastYrOutstandingPurch.frx":0594
      TabIndex        =   1
      Text            =   "cbo"
      Top             =   720
      Width           =   1980
   ...
   Begin VB.Label Label6 
      BackColor       =   &H00C00000&
      Caption         =   " Last Year Outstanding Purchase"
      BeginProperty Font 
         Name            =   "Times New Roman"
         Size            =   15.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmUtlLastYrOutstandingPurch"
...
Case 4 'Find
    If gBackEndDB = gBackEndAccess Then '--Access
        gClsSearch.SearchMultiField "tblOutstanding,tblMastAccount", "VNo,Vtype,Vyear,iif(VType='PY','Trade','Sale In Transit'),VDt,OutBillNo,AcName,OutAmount,OutItShort,OutTicket", Array("VNo", "Vtype", "Type", "Year", "Date", "Bill No", "Party", "Amount", "Short", "Ticket"), Array(800, 0, 1500, 1500, 1200, 1000, 1800, 1200, 1000, 1000), "  outaccode=AcCode and Vyear<=" & gCYear - 1 & " and vtype in ('PY','PT') ", "", "Vdt,Vno", 0, 0
    Else '--Oracle
    ...
    If gClsSearch.SearchMultiRetCol(1) <> "" Then
        rstbl.CancelUpdate
        rstbl.MoveFirst
        FindStr = gClsSearch.SearchMultiRetCol(0) & "-" & gClsSearch.SearchMultiRetCol(1) & "-" & gClsSearch.SearchMultiRetCol(2)
        rstbl.Find "VnoVTypeVyear='" & FindStr & "'"
...
Case 5 'List
        ...
        cryReport.SelectionFormula = "{tblOutStanding.VYear} = " & gCYear - 1 & " and ({tblOutStanding.Vtype}='SO' or {tblOutStanding.Vtype}='PY' or {tblOutStanding.Vtype}='PT'"
...
Private Sub Form_Load()
    ...
    ConditionStr = " where Vyear<=" & gCYear - 1 & " and Vtype in ('PT','PY')"
    If gBackEndDB = gBackEndAccess Then
        X = "Select " & tblName & ".*,Vno & '-' & Vtype & '-' & VYear  as VnoVTypeVyear from " & tblName & ConditionStr
    Else '-- Oracle
...
If Index = 3 Then
    If (Year(mskDt(3)) = Year(gCYSDate)) Then
      txt(2) = Year(mskDt(3)) - 1
    Else
      txt(2) = Year(mskDt(3))
    End If
...
'--Party
  If Index = 5 Then
    gClsSearch.SearchMultiField "QryMastaccount", "Account,AcName,AcCity,AgName", Array("Party", "AcName", "City", "Group"), Array(3000, 0, 2000, 3000), "(AgCode=90015 or GpCode=90015 or AgCode=90029 or GpCode=90029)", Chr(KeyAscii), "AcName", LeftPos, TopPos, True
    KeyAscii = 0
...
End If
 ...
 If Index = 11 Then
    ...
    ElseIf Cbo(1).ListIndex = 1 Then '--SIT
...
If CLng(txt(0)) = 0 Or Trim(txt(4)) = "" Or Trim(txttemp(5)) = "" Or Trim(txttemp(6)) = "" Or Trim(txttemp(11)) = "" Then
