...
x = "select  IntDbNtDate,IntDbNtAmt,IntDbNtRecdAmt," & IIf(gBackEndDB = gBackEndAccess, "iif(IsIntDbNtComplete=1,'Y','')", "decode(IsIntDbNtComplete,1,'Y','')") & " ,AdjNo from tblRecVsSale where " _
...
With msGrid
    .Clear
    .FormatString = "< Date            |>                Amount|>   Received Amount|<Is DbNt Complete (Y)     |<AdjNo"
    .ColWidth(4) = 0
    .Rows = 2
    'FillRelatedRecord
End With
...
If LCase(Me.ActiveControl) <> "txtgrid" Then msGrid.Col = 2
...
With msGrid
    i = 1
    Do While i < .Rows
        AdjNo = CLng(.TextMatrix(i, 4)) '--AdjNo
        AmtRecd = CDbl(.TextMatrix(i, 2)) '--Recd Amt
        IsComplete = IIf(Trim(UCase(.TextMatrix(i, 3))) = "Y", 1, 0) '- Is Complete
