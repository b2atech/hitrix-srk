...
With CrystalReport1
     ...
     Select Case gSelectedMenu
       Case "Outsading To Broker", "Outsading To Party"
             ...
             gSetupCn.BeginTrans
             gSetupCn.Execute "Delete from tmptbloutstanding where username = '" & gUserName & "' and tmptbloutstanding.OutDueDate >  cdate('" & Format(DTPicker2.Value, "yyyy/MM/dd") & "')"
             ...
             Do While Not RsTmp.EOF
                FCode = GProcGetColumnValue("TblMastAccount", "AcName", Trim(RsTmp.Fields(0)), "S", "AcCode", "N")
                With CrystalReport1
...
'                   FileNM = "M" + "" & Format(Now, "ddMMyyyyhhmmss") & "" & ".pdf"
                   FileNM = "DueList" + Trim(CStr(FCode)) + "-" + Trim(CStr(i)) + ".pdf"
'                   FileNM = PtyGstin + "-" + gCGSTIN + "-" + Trim(CStr(i)) + ".pdf"
...
'                   PtyMbl = "9922997663"
                   If PtyMbl = "" Then
                      ...
                      gCn.Execute "insert into tmpWhatsAppErr (Mbl1,PartyName,Error,USERNAME) values " _
                                  & " ('-','" & PtyName & "','Phone No not found in firm '+'" & gCName & "','" & gUserName & "')"
                   Else
