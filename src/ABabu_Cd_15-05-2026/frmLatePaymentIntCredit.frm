...
Begin VB.Form frmLatePaymentIntCredit 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00FBF2E1&
   BorderStyle     =   0  'None
   ...
   ClientTop       =   0
   ClientWidth     =   14520
   Icon            =   "frmLatePaymentIntCredit.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.OptionButton OptType 
         BackColor       =   &H00FFC0C0&
         Caption         =   "Credit Note Check List"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   9.75
            Charset         =   0
   ...
   Begin Crystal.CrystalReport cryReport 
      Bindings        =   "frmLatePaymentIntCredit.frx":058A
      Left            =   -45
      Top             =   660
      _ExtentX        =   741
      _ExtentY        =   741
   ...
   Begin VB.Label lbl1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "INTEREST JV NO ( In Depot )  :"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   0
   ...
   Begin VB.Label Label6 
      BackColor       =   &H00EFB17A&
      Caption         =   "   Payment Interest Credit  Note List"
      BeginProperty Font 
         Name            =   "Times New Roman"
         Size            =   15.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmLatePaymentIntCredit"
...
Case 4 'Find
    gClsSearch.SearchMultiField "tblVoucher", "VnoList,Vdt,sum(InterestCredit)", Array("Vno", "Vdt", ""), Array(1400, 1300, 0), CriteriaStr + " group by VnoList,Vdt", , , , , True
...
Case 0, 1:
    ...
    Do While Vno <= txtno(1)
        X = "select VBillNo,VBillType, VBillDate,VBillAmt,tblMastAccount.AcOurCode,tblMastAccount.AcName,tblMastAccount.AcCity,tblMastAccount_Br.AcOurCode,tblMastAccount_Br.AcName, VAmt,DueOnDt,tblVoucher.Vdt,LateDays,Interest,JvNoInt,Grace+GraceExtra,IntRt,DbNtMonthly,'" & gUserName & "'" _
            ...
            gCn.Execute " insert into TmpGentbl (VBillNo,VBillType,VBillDt,Amt1,PartyCode,PartyName,Nar1,BrokerCode,BrokerName,Amt2,Dt1,Vdt,Amt3,Amt4,Amt5,Amt6,amt7,amt8,UserName) " & X
...
Private Sub Form_Load()
    ...
    Else
       MainFirm = X
    End If
    CriteriaStr = " VType = '" & frmTypeStr & "' and  VnoList <> 0 and InterestCredit > 0   and VFirm  = '" & gCCode & "' and VYear=" & gCYear
...
If FormAction = vbDataActionAddNew And Index = 38 Then
   PreperReport
End If
...
With msGrid
     For i = 1 To .rows - 1
         ...
         If Val(.TextMatrix(i, 11)) > 0 Then
            ChkTds = Round((Val(.TextMatrix(i, 10)) * Val(txt(42))) / 100, 0)
...
X = "select VBillNo,VBillType, VBillDate,VBillAmt,tblMastAccount.AcName,tblMastAccount_Br.AcName, VAmt,tblVoucher.Vdt,LateDays,InterestCredit,TDSOn,LessTDS,DBNtInt,DbNtInBillNo,DbNtMonthly,DueOnDt,tblVoucher.Vno,VCtrNo,Grace,IntRt" _
& " from tblVoucher,tblMastAccount,tblMastAccount tblMastAccount_Br,tblSale where " _
...
'19 - IntRt
    ...
    With msGrid
        Select Case .Col
            Case 0, 1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 10
                txtGrid.Locked = True
            Case Else
...
With msGrid
    ...
    If KeyAscii = 13 Then
        KeyAscii = 0
        If .Col = rsRecVsSale.Fields.Count - 11 Then
...
With msGrid
   ...
   End If
    ...
    If KeyAscii = 13 And .Col < rsRecVsSale.Fields.Count - 11 Then '- -4
        If .Col <= 8 Then
           .Col = 9
        Else
            .Col = .Col + 1
        End If
    ElseIf KeyAscii = 13 And .Col = rsRecVsSale.Fields.Count - 11 Then '- -4
        If .Row = (.rows - 1) Then
            .rows = .rows + 1
            .Row = .Row + 1
        Else
...
If (GProcRstOpen(RsP, Y, "O") > 0) Then
    ...
    JvNoDepot = RsP.Fields("JvNoIntInDepot")
End If
...
End With
DoEvents
'--- For delete Int JV In Depot Firm
Y = "select VCtrNo from tblVoucher where VType='JV' and VNo=" & JvNoDepot & " and VFirm  = '" & gCCode & "' and  VYear=" & gCYear & "  order by VCtrNo"
i = GProcRstOpen(RsP, Y, "R")
With RsP
     If Not .EOF Then
       .MoveFirst
       Do While Not .EOF
          gCn.Execute "Delete from tblvoucher where VType = 'JV' and VFirm = '" & gCCode & "'  and VYear = " & Val(txt(3)) & "  and VCtrNo = " & RsP.Fields(0) & "   and  vno = " & JvNoDepot
          .MoveNext
       Loop
     End If
End With
...
'End With
DoEvents
gCn.Execute "update tblVoucher set InterestCredit = 0, LessTDS = 0 ,DueOnDt =  '', VnoList = 0 ,Grace = 0,IntRt = 0,LateDays = 0,JvNoInt=0 ,JvNoLessTDS = 0,GraceExtra =0,IntFromDate = null,IntParty = 0,TdsRt = 0,TDSOn=0,DBNtInt = null,DbNtMonthly = 0,DbNtInBillNo = null " _
...
With msGrid
    ...
    Do While i < .rows
        ...
        If CDec(Val(.TextMatrix(i, 9))) > 0 Then
           gCn.Execute "update tblVoucher set InterestCredit = " & Val(.TextMatrix(i, 9)) & ", DueOnDt = '" & IIf(IsDate(.TextMatrix(i, 15)), Format(.TextMatrix(i, 15), "yyyy/mm/dd"), "") & "', VnoList = " & Val(txt(35)) & " ,Grace = " & Val(.TextMatrix(i, 18)) & ",IntRt = " & Val(.TextMatrix(i, 19)) & ",LateDays = " & Val(.TextMatrix(i, 8)) & ",  " _
                       ...
                       & " JvNoInt=" & Val(txt(32)) & ",JvNoIntInDepot=" & Val(txt(48)) & "  " _
...
Y = "Select * from " & tblName & " where VnoList = " & txt(35) & " and " & CriteriaStr & " "
DoEvents
Y = "Select * from " & tblName & " where VnoList = " & txt(35) & "  and " & CriteriaStr & " order by VCrAcCode,DbNtInBillNo"
If (GProcRstOpen(RsP, Y, "O") > 0) Then
    wParty = 0
    If Val(txt(32)) > 0 Then
       wJvNo = txt(32)
    ...
    If MainFirm <> gCCode Then
        If Val(txt(48)) > 0 Then
           wJvNoDepot = txt(48)
        Else
           wJvNoDepot = GProcGenerateIdMonthwise("TblVoucher", "Vno", Mon, "Vdt", "Vtype='JV' and VFirm = '" & gCCode & "' and VYear=" & gCYear)
        End If
    End If
    i = 1
    J = 1
    ...
    Do While Not RsP.EOF
       ...
       If wIntAmt > 0 Then
            ...
            i = i + 1
            If MainFirm <> gCCode Then
               rstbl!VCrAcCode = WcrNtAc '--   gCDepotMainFirm
               rstbl!VDrAcCode = gLatePayIntAcCodePay
            Else
            ...
            rstbl!VTopCrDr = 1
            rstbl!vamt = wIntAmt
            rstbl!VNar1 = "Earlier Payment Credited "
            rstbl!VNar2 = "Agt. Bill No : " & wBlNo
            rstbl!VNar3 = ""
            rstbl!VNAR4 = ""
            rstbl!InterestCredit = wIntAmt
            rstbl!VIsRefEntType = 1
            If MainFirm = gCCode Then
                rstbl!VBillNo = wBlNo
            ...
            DoEvents
        End If
        If MainFirm <> gCCode And wIntAmt > 0 Then
            rstbl.AddNew
            rstbl!Vno = wJvNoDepot '--VNo
            rstbl!Vtype = "JV"  '--Vtype
            rstbl!Vdt = CDate(txt(2)) '--Vdt(2)
            rstbl!VYear = txt(3) '--VYear
            ...
            i = i + 1
            rstbl!VCrAcCode = wParty
            rstbl!VDrAcCode = gCDepotMainFirm
            rstbl!VTopCrDr = 1
            rstbl!vamt = wIntAmt
            rstbl!VNar1 = "Earlier Payment Credited "
            rstbl!VNar2 = "Agt. Bill No : " & wBlNo
            ...
            rstbl!VBillFirm = gCCode
            rstbl.Update
            gCn.Execute "update tblVoucher set JvNoIntInDepot = " & wJvNoDepot & " " _
                        & " where VCrAcCode = " & wParty & " and  VnoList = " & txt(35) & " and " & CriteriaStr & ""
            DoEvents
        End If
        '--- TDS Db Nt.
...
'            rstbl!VCrAcCode = gTDSAcCodePay
'            rstbl!VTopCrDr = 0
'            rstbl!vamt = wTdsAmt
'            rstbl!VNar1 = "T D S @ " + CStr(gTDSRate) + " % On Int. Of Rs. " + CStr(wTdsOn) + " Credited"
'            rstbl!VNar2 = "Agt. Bill No : " & wBlNo
'            rstbl!VNar3 = ""
'            rstbl!VNAR4 = ""
...
lblInt2.Caption = Format(BnkTot, "###0.00")
...
If GProcRstOpen(RsTmp, X, "R", gCn) > 0 Then
   ...
   Do While Not RsTmp.EOF
      With msGrid
           If Val(RsTmp.Fields(15)) < Val(txt(37)) Then
                J = .rows - 1
                ...
                .TextMatrix(J, 10) = 0
                .TextMatrix(J, 11) = 0 '--Round((.TextMatrix(J, 10) * Val(txt(42))) / 100, 0)         '11 - Tds
...
'If OldTdsOn <> Val(msGrid.TextMatrix(msGrid.Row, 9)) Then
...
'    msGrid.TextMatrix(msGrid.Row, 11) = Round((msGrid.TextMatrix(msGrid.Row, 10) * Val(txt(42))) / 100, 0)
