...
Begin VB.Form frmBookingMillBillDespatchNew 
   BackColor       =   &H00FBF2E1&
   BorderStyle     =   0  'None
   ...
   FillColor       =   &H000040C0&
   FillStyle       =   0  'Solid
   Icon            =   "frmBookingMillBillDespatchNew.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.ComboBox Cbo 
      ...
      Height          =   390
      Index           =   22
      ItemData        =   "frmBookingMillBillDespatchNew.frx":058A
      Left            =   4740
      List            =   "frmBookingMillBillDespatchNew.frx":0594
      TabIndex        =   11
      Text            =   "Cbo(22)"
      Top             =   4320
      Width           =   2475
   ...
   Begin Crystal.CrystalReport cryReport 
      Bindings        =   "frmBookingMillBillDespatchNew.frx":05A7
      Left            =   4110
      Top             =   0
      _ExtentX        =   741
      _ExtentY        =   741
   ...
   Begin MSFlexGridLib.MSFlexGrid msGrid 
      ...
      EndProperty
   End
   Begin MSFlexGridLib.MSFlexGrid msGrid2 
      Height          =   2955
      Left            =   1095
      TabIndex        =   90
      Top             =   6375
   ...
   Begin VB.Label Label7 
      BackStyle       =   0  'Transparent
      Caption         =   "Delivery Shedule Details"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   0
   ...
   Begin VB.Label Label6 
      BackColor       =   &H00EFB17A&
      Caption         =   "     Despatch Details   ( Mill Bill Booking )"
      BeginProperty Font 
         Name            =   "Times New Roman"
         Size            =   15.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmBookingMillBillDespatchNew"
...
Dim rstblSub2 As Recordset '-- TblPurchSub
...
Case 11 'Find ' QryMastAccount
     gClsSearch.SearchMultiField "qryPendingDespatchDetailPurchase", "Code,Mill,ItName,Vno,Vdt,BkDespDate,BkDespBag,Vfirm,Vtype,Vyear", Array("Code", "Mill Name", "Count Name", "Book.No", "Date", "Desp.Dt.", "Desp.Bags", "", "", ""), Array(1000, 3000, 3000, 1500, 1500, 1500, 1500, 1500, 0, 0, 0), " vtype='" & frmTypeStr & "'  and VFirm = '" & gCCode & "' and VYear=" & gCYear, "", "Code,Mill,ItName,Vno,Vdt", 0, 0
     If gClsSearch.SearchMultiRetCol(0) <> "" Then
        GProcRstOpen rstbl, "Select * from " & tblName & " where " & IdField & " = " & gClsSearch.SearchMultiRetCol(3) & " and " & Criteria, "R"
...
Case 4 'Find ' QryMastAccount
    ...
    End If
Case 5 'Cancel Bags
    gClsSearch.SearchMultiField "tblBooking,TblMastAccount", "Vno,AcName,Vdt", Array("Vno", "Party", "Vdt"), Array(txt(0).Width, 4000, 1080), " tblBooking.BkParty=TblMastAccount.AcCode and vtype='" & frmTypeStr & "'  and VFirm = '" & gCCode & "'  and VYear=" & gCYear, "", "Vno,Vdt", 0, 0
    If gClsSearch.SearchMultiRetCol(0) <> "" Then
        GProcRstOpen rstbl, "Select * from " & tblName & " where " & IdField & " = " & gClsSearch.SearchMultiRetCol(0) & " and " & Criteria, "R"
    ...
    Call GProcActivateControls(Me, True)
    Call GProcSetButtons(Me, vbDataActionUpdate)
    mskDt(29).Enabled = True
    txt(27).Enabled = True
    txt(28).Enabled = True
    mskDt(29).SetFocus
Case 6 'Add
    lblPartyCode.Caption = ""
    lblBrokCode.Caption = ""
...
Cbo(22).Enabled = False
If frmTypeStr = "OT" Then
   cmdBtn(11).Visible = True
...
Dim mNo As Integer
''--- Voucher Sub
'gCn.Execute "delete from tblBooKingDesp where vtype='" & frmTypeStr & "' and vno=" & txt(0) & " and Vfirm = '" & gCCode & "'  and " _
'& " VYear=" & gCYear
...
With msGrid
    ...
    Do While i < .rows
        If Val(.TextMatrix(i, 2)) > 0 Then
           If frmTypeStr = "OM" Then
              Y = Val(txt(0)) & ",'" & txt(1) & "','" & Format(txt(2), "yyyy/mm/dd") & "'," & Val(txt(3)) & ",'" & gCCode & "'," & i & "," & Val(.TextMatrix(i, 0)) & ",'" & IIf(IsDate(.TextMatrix(i, 1)), Format(.TextMatrix(i, 1), "yyyy/mm/dd"), "") & "',"
              Y = Y & Val(.TextMatrix(i, 2)) & ",'" & IIf(IsDate(.TextMatrix(i, 3)), Format(.TextMatrix(i, 3), "yyyy/mm/dd"), "") & "'," & Val(.TextMatrix(i, 4)) & "   "
              gCn.Execute "Insert into tblBooKingDesp(vno,vtype,vdt,vyear,vfirm,BkSrNo,BkDespItSrNo,BkDespDate,BkDespBag,BkDespShDate,BkDespTmpNo1)" _
                          & " Values(" & Y & ")"
           Else
              Y = Val(txt(0)) & ",'" & txt(1) & "','" & Format(txt(2), "yyyy/mm/dd") & "'," & Val(txt(3)) & ",'" & gCCode & "'," & i & "," & Val(.TextMatrix(i, 0)) & ",'" & IIf(IsDate(.TextMatrix(i, 1)), Format(.TextMatrix(i, 1), "yyyy/mm/dd"), "") & "',"
              Y = Y & Val(.TextMatrix(i, 2)) & ",'" & IIf(IsDate(.TextMatrix(i, 3)), Format(.TextMatrix(i, 3), "yyyy/mm/dd"), "") & "','" & (.TextMatrix(i, 4)) & "','" & UCase(.TextMatrix(i, 5)) & "'   "
              gCn.Execute "Insert into tblBooKingDesp(vno,vtype,vdt,vyear,vfirm,BkSrNo,BkDespItSrNo,BkDespDate,BkDespBag,BkDespShDate,BkDespTmpChr2,BkDespTmpChr1)" _
                          & " Values(" & Y & ")"
           End If
'            rstSub.AddNew
...
'               rstSub!BkDespDate = CDate(.TextMatrix(i, 1)) '--SH.Date
'            Else
'               rstSub!BkDespDate = CDate(txt(2))  '--Des.Date
'            End If
'            rstSub!BkDespBag = CLng(.TextMatrix(i, 2)) '--Bag(2)
'            If IsDate(.TextMatrix(i, 3)) = True Then
'               rstSub!BkDespShDate = CDate(.TextMatrix(i, 3)) '--SH.Date
''            Else
' '              rstSub!BkDespShDate = ""  '--Des.Date
'            End If
'            rstSub.Update
        End If
        i = i + 1
...
If frmTypeStr = "OM" Then
    X = "select BkDespItSrNo,BkDespDate,BkDespBag,BkDespShDate,BkDespTmpNo1" _
...
Else
    X = "select BkDespItSrNo,BkDespDate,BkDespBag,BkDespShDate,BkDespTmpChr2,BkDespTmpChr1" _
    & "  from tblBooKingDesp where " _
    & "  tblBooKingDesp.vtype='" & frmTypeStr & "' and tblBooKingDesp.vno=" & txt(0) & " and tblBooKingDesp.VFirm = '" & gCCode & "' and " _
    & "  tblBooKingDesp.VYear=" & gCYear & " order by BkSrNo"
End If
...
With msGrid
   If frmTypeStr = "OM" Then
      .FormatString = ">Sh.Sr.No.|<Desp. Date|>Bag   |<Sch.Date     |<Recd"
   Else
      .FormatString = ">Sh.Sr.No.|<Desp. Date|>Bag   |<Sch.Date     |<Lot No.         |<Recd"
   End If
   If frmTypeStr = "OM" Then
     .ColWidth(4) = 0
   End If
...
Dim X As String
Set rstblSub2 = New Recordset
X = "select BkSrNo,BkShDate,BkShBag" _
...
GProcRstOpen rstblSub2, X, "R"
With msGrid2
   .FormatString = ">Sr|<Date            |>Bag        "
    If FormAction = vbDataActionAddNew Then
        .rows = 2
'        GridAddNew
    Else
...
If Index = 0 Then
   ...
   If rstbl.EOF = True Then
      MsgBox "Booking No Not Found", vbCritical + vbOKOnly, Me.Caption
...
If KeyAscii = 13 Then
    KeyAscii = 0
    With msGrid
        If (.Col = 2 And frmTypeStr = "OM") Or (.Col = 5 And frmTypeStr = "OT") Then    'rstblSub.Fields.Count - 2 Then
...
If IsDate(msGrid.TextMatrix(msGrid.Row, 1)) = False Then
    msGrid.TextMatrix(msGrid.Row, 1) = Format(Now, "dd/MM/yyyy")
End If
If msGrid.TextMatrix(msGrid.Row, 0) = 0 Then
   msGrid.TextMatrix(msGrid.Row, 0) = 1
End If

'If msGrid.TextMatrix(msGrid.Row, 0) > 0 And msGrid.Col = 0 Then
'    If IsDate(msGrid.TextMatrix(msGrid.Row, 3)) = False Then
'       X = "Select BkShDate from tblBooKing where vno=" & txt(0) & " and BkSrNo =  " & msGrid.TextMatrix(msGrid.Row, 0) & " And " & Criteria
'       GProcRstOpen Rs1, X, "R"
'       If Rs1.EOF Then
            ...
            Select Case i
                Case 1 '-- Date
                    ...
                    If IsDate(.TextMatrix(RowIndex, i)) = True Then
                       If CDate(.TextMatrix(RowIndex, i)) < mskDt(2) Then
                           MsgBox "Despatch Date Must Be Greater Than Booking Date.", vbCritical + vbOKOnly, Me.Caption
                           .Col = i
                           msGrid.SetFocus
                        Exit Function
                       End If
                    End If
'                    If msGrid2.TextMatrix(1, 1) <> "" Then
'                        X = "Select BkSubBag,tblBooKingSub.BkSubDDate from tblBooKingSub where vno=" & txt(0) & " And  format(tblBooKingSub.BkSubDDate,'dd/MM/yyyy') = '" & Format(.TextMatrix(RowIndex, 3), "dd/MM/yyyy") & "'  And " & Criteria
'                        GProcRstOpen Rs1, X, "R"
'                        If Rs1.EOF And IsNull(Rs1.Fields(1)) = False Then
'                               MsgBox "Please Chech Sh. Date ", vbCritical + vbOKOnly, Me.Caption
'                               .Col = i
'                               msGrid.SetFocus
'                            Exit Function
'                        End If
...
'                    If msGrid2.TextMatrix(1, 1) <> "" Then
'                        If CDbl(.TextMatrix(RowIndex, 2)) > 0 And IsDate(.TextMatrix(RowIndex, i)) = False Then
'                             MsgBox "Check Sh.Date.", vbCritical + vbOKOnly, Me.Caption
'                             .Col = i
'                             msGrid.SetFocus
'                             Exit Function
'                         End If
'                         X = "Select BkSubBag from tblBooKingSub where vno=" & txt(0) & " And  format(tblBooKingSub.BkSubDDate,'dd/MM/yyyy') = '" & Format(.TextMatrix(RowIndex, i), "dd/MM/yyyy") & "'  And " & Criteria
'                         GProcRstOpen Rs1, X, "R"
'                         If Rs1.EOF Then
'                                MsgBox "Please Chech Sh. Date ", vbCritical + vbOKOnly, Me.Caption
'                                .Col = i
...
lblPartyCode.Caption = GProcGetColumnValue("tblMastAccount", "AcCode", txt(5), "N", "AcOurCode", "S")
lblBrokCode.Caption = GProcGetColumnValue("tblMastAccount", "AcCode", txt(7), "N", "AcOurCode", "S")
...
With rstblSub2
    If Not .EOF Then
        ...
        msGrid2.rows = 2
        Do While Not .EOF
            For ColIndex2 = 0 To .Fields.Count - 1
                Select Case .Fields(ColIndex2).Type
                    Case adInteger, 2, 3 '-- Integer
                        msGrid2.TextMatrix(RowIndex2, ColIndex2) = CLng(.Fields(ColIndex2))
                    Case adDecimal, 131, adNumeric, adDouble  '-- Double
                        msGrid2.TextMatrix(RowIndex2, ColIndex2) = Format(.Fields(ColIndex2), GProcNumberFormat(.Fields(ColIndex2).Precision, .Fields(ColIndex2).NumericScale))
                    Case Else
                        msGrid2.TextMatrix(RowIndex2, ColIndex2) = .Fields(ColIndex2) & vbNullString
                End Select
'                If ColIndex = 9 Then TotBag = TotBag + CLng(msGrid.TextMatrix(RowIndex, 9)) '-- Tot Bag
'                If ColIndex = 10 Then TotWt = TotWt + CDbl(msGrid.TextMatrix(RowIndex, 10)) '-- Tot Wt
            Next ColIndex2
            ...
            .MoveNext
            If Not .EOF Then msGrid2.rows = msGrid2.rows + 1
        Loop
    Else
        msGrid2.rows = msGrid2.rows + 1
'        ReNumberGridSrno
    End If
End With
