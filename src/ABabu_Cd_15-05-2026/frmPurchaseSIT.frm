...
Begin VB.Form frmPurchaseSIT 
   ...
   Begin Crystal.CrystalReport cryReport 
      Bindings        =   "frmPurchaseSIT.frx":058A
      Left            =   4110
      Top             =   0
      _ExtentX        =   741
      _ExtentY        =   741
   ...
   Begin MSComCtl2.DTPicker dtpDt 
      ...
      CurrentDate     =   -273
   End
   Begin VB.Label lblCheckLrDetail 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "Check L.R.Details                   "
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   14.25
         Charset         =   0
   ...
   Begin VB.Label Label6 
      BackColor       =   &H00EFB17A&
      Caption         =   "     Sales In Transit  Purchase"
      BeginProperty Font 
         Name            =   "Times New Roman"
         Size            =   15.75
         Charset         =   0
...
Dim TotBag As Long
Dim TotWt As Double
Dim TotLrdBag As Long
Dim TotLrdWt As Double
...
Case 4 'Find ' QryMastAccount
    ...
    End If
Case 5 '--Lr Details
    FormAction = vbDataActionFind
    gfrmBagInwVNo = txt(0)
    gfrmBagInwVType = frmTypeStr
    gfrmBagLrDt = dtpDt(10)
    frmLrDetailEntry.Show
    DoEvents
...
'    dtpDt(2) = Format(Now, "dd/MM/yyyy")
    '--A/C
    txttemp(6) = GProcGetColumnValue("tblMastAccount", "AcCode", CStr(gAcCodePT), "N", "AcName", "S")
    txttemp(35) = GProcGetColumnValue("tblMastTransactionCode", "TranCode", CStr(gTranCdPT), "S", "TranCode", "S")
...
Case 8 'Delete
    ...
    'If mURecDel = True Then
      If IsRelatedGatePass = True Then
            ...
            Exit Sub
      End If
      X = "Select LrDBag,LrDWt,SlBillNo from tblSitLrDetails where  SlVno > 0 and vtype='" & frmTypeStr & "' and vno=" & txt(0) & " and Vfirm = '" & gCCode & "' and  VYear=" & gCYear
      GProcRstOpen Rs1, X, "R"
      If IsNull(Rs1.Fields(0)) = True Then
         MsgBox "Related Sale Bill No " + Rs1.Fields(2) + " Exist You can Delete this record ", vbCritical + vbOKOnly, App.Title
         Exit Sub
      End If
      If IsRelatedRecord = False Then
        If MsgBox("Do You Really Delete This Record?", vbQuestion + vbYesNo + vbDefaultButton1, "Confirm Delete") = vbYes Then
           gCn.Execute "delete from tblPurchSub where vtype='" & frmTypeStr & "' and vno=" & txt(0) & " and Vfirm = '" & gCCode & "'  and " _
            ...
            '--- LR Details
            gCn.Execute "delete from tblSitLrDetails where vtype='" & frmTypeStr & "' and vno=" & txt(0) & " and Vfirm = '" & gCCode & "'  and " _
                        & " VYear=" & gCYear
...
With msGrid
    ...
    Do While i < .rows
       ...
       If Val(.TextMatrix(i, 12)) = 0 Then
           ...
           X = X & wit & ","
           X = X & Val(.TextMatrix(i, 6)) & "," & Val(.TextMatrix(i, 8)) & "," & Val(.TextMatrix(i, 9)) & "," & Val(.TextMatrix(i, 10)) & "," & Val(.TextMatrix(i, 11)) & ",'" & .TextMatrix(i, 5) & "',"
           X = X & wgd
       ...
       Else
          ...
          gCn.Execute "Update TblPurchSub set pursubitsrno=" & i & ", vdt ='" & Format(txt(2), "yyyy/mm/dd") & "',PursubBookNo=" & Val(.TextMatrix(i, 1)) & ",PurSubBookDt='" & IIf(IsDate(.TextMatrix(i, 2)), Format(.TextMatrix(i, 2), "yyyy/mm/dd"), "") & "'," _
                & "PurSubItCode=" & wit & ",PurSubBag=" & Val(.TextMatrix(i, 6)) & ",PurSubwt=" & Val(.TextMatrix(i, 8)) & ",PurSubRt=" & Val(.TextMatrix(i, 9)) & ",PurSubRtPer=" & Val(.TextMatrix(i, 10)) & ",PurSubAmt=" & Val(.TextMatrix(i, 11)) & ",PurSubLotNo='" & Trim(.TextMatrix(i, 5)) & "',PurSubGodown=" & wgd & " where PurSubItCtrlNo=" & Val(.TextMatrix(i, 12))
       End If
       i = i + 1
    Loop
End With
...
If FormAction = vbDataActionFind Then ReadFields
End Sub
...
X = "select PurSubItSrNo,PurSubBookNo,PurSubBookDt,ItName,Narration,PurSubLotNo,PurSubBag,ItStdpack,PurSubWt" _
...
With msGrid
   .FormatString = ">Sr|<Bk.No    |<Book Date  |<Count                                                  |<Godown               |<Lot No      |>Bag      |>Std.Pack.|>Weight               |>Rate                |>Per    |>Amount                   ||"
   .ColWidth(1) = 0
   .ColWidth(2) = 0
   .ColWidth(4) = 0
   .ColWidth(5) = 0
...
Select Case Index
    Case 5  '-- Party  Code
         ...
         lblTinNo.Caption = GProcGetColumnValue("tblMastAccount", "AcOurCode", UCase(txtCode(5)), "S", "AcBST", "S")
...
'        gClsSearch.SearchMultiField "tblMastaccount,tblMastGroup", "AcName,AgName,AcContPer,AcCity,AcBST,AcMsOmsParty", Array("Party", "Group", "Contact Person", "City", "TIN", "Ms/Oms"), Array(5000, 3000, 3000, 2000, 0, 0), "(tblMastaccount.Agcode in (90015,90029) ) and tblMastaccount.AgCode = tblMastGroup.AgCode ", Chr(KeyAscii), "", LeftPos, TopPos, True
        gClsSearch.SearchMultiField "QryMastAccount", "Account,AcOurCode,CGName,AcContPer,AcCity,AgName,AcBST,AcMsOmsParty", Array("Account Name", "Code", "Party Group", "Contact Person", "City", "Group", "TIN", "Ms/Oms"), Array(4000, 1000, 2000, 2000, 2000, 2500, 0, 0), "(Agcode in (90015,90029) )", Trim(Chr(KeyAscii)), "", LeftPos, TopPos, True
        KeyAscii = 0
        lblTinNo.Caption = ""
        lblMsOms.Caption = ""
        txtCode(5) = ""
...
With msGrid
    If KeyAscii = 27 Then '----Esc Key
        KeyAscii = 0
        If ((Val(.TextMatrix(.Row, 6)) = 0) Or (Val(.TextMatrix(.Row, 11)) = 0)) And .rows > 2 Then
            .RemoveItem (.Row)
            txtGrid.text = ""
        End If
        txtGrid.Visible = False
...
msGrid.TextMatrix(msGrid.Row, 7) = StdPack
...
If CDbl(msGrid.TextMatrix(msGrid.Row, 8)) > 0 Then
    msGrid.TextMatrix(msGrid.Row, 9) = CDbl(msGrid.TextMatrix(msGrid.Row, 11)) * CDbl(msGrid.TextMatrix(msGrid.Row, 10)) / CDbl(msGrid.TextMatrix(msGrid.Row, 8))
Else
    msGrid.TextMatrix(msGrid.Row, 9) = 0
End If
msGrid.TextMatrix(msGrid.Row, 9) = Format(msGrid.TextMatrix(msGrid.Row, 9), LStr)
...
'                    gClsSearch.SearchMultiField "tblMastItem,tblMastaccount", "ItName,AcName,ItStdpack,ItStdrateper", Array("Count Name", "Mill", "Std. Pack", "Rate Per"), Array(txtGrid.Width, 2000, 1000, 1000), "tblMastItem.ITMillCode=tblMastaccount.acCode and ITMillCode=" & MillCode, Chr(KeyAscii), "ItName", LeftPos, TopPos, True
   ...
   End If
    ...
    End If
    '--- Navigate Row ,Col
    If KeyAscii = 13 And .Col < 11 Then 'rstblSub.Fields.Count - 2 Then
        .Col = .Col + 1
'        If .Col = 6 Then
...
'            End If
'        End If
    ElseIf KeyAscii = 13 And .Col = 11 Then '= rstblSub.Fields.Count - 2 Then
        If .Row = (.rows - 1) Then
            .rows = .rows + 1
            .Row = .Row + 1
            GridAddNew
            ...
            Select Case i
                Case 3 '-- Count
                 '-Rate=0 ,count,wt=0 then delete row
                    If RowIndex > 1 And Trim(.TextMatrix(RowIndex, 3)) = "" And CDbl(.TextMatrix(RowIndex, 8)) = 0 And CDbl(.TextMatrix(RowIndex, 9)) = 0 Then
                        .RemoveItem (RowIndex)
                        GoTo Validate
                    End If
...
Cbo(22) = GProcGetColumnValue("tblMastNarration", "NarrCode", CStr(gTaxCodePT), "N", "Narration", "S")
...
'              Case "45"
'                   MsgBox "Required Data Tax = 'C S T Against Form C', Tin No Should Not Balnk And M.S. Party", vbInformation + vbOKOnly, Me.Caption
'       End Select
'       txttemp(35).SetFocus
'       Exit Function
'
...
'    End If
    ...
    End If
    '--- Transport
    If txttemp(11) = "" Or txttemp(11) = "-" Then
    ...
    If Not .EOF Then
        ...
        Do While Not .EOF
            For ColIndex = 0 To .Fields.Count - 1
                ...
                If ColIndex = 8 Then TotWt = TotWt + CDbl(msGrid.TextMatrix(RowIndex, 8)) '-- Tot Wt
            Next ColIndex
            RowIndex = RowIndex + 1
            .MoveNext
...
X = "Select sum(LrDBag),sum(LrDWt) from tblSitLrDetails where vtype='" & frmTypeStr & "' and vno=" & txt(0) & " and Vfirm = '" & gCCode & "'  and  VYear=" & gCYear
GProcRstOpen Rs1, X, "R"
If IsNull(Rs1.Fields(0)) Or IsNull(Rs1.Fields(1)) Then
   TotLrdBag = 0
   TotLrdWt = 0
Else
   TotLrdBag = Rs1.Fields(0)
   TotLrdWt = Rs1.Fields(1)
End If
If (TotBag - TotLrdBag = 0) And (TotWt - TotLrdWt = 0) Then
    lblCheckLrDetail.Visible = False
Else
    lblCheckLrDetail.Visible = True
End If
Rs1.Close
SetRelatedRS
End Sub
...
If IsDate(dtpDt(2)) Then mskDt(32) = DateAdd("d", txt(31), dtpDt(2))
'---- Sub amt
CheckForNumberGrid
CheckForNumber
RowIndex = 1
With msGrid
    Do While RowIndex < .rows
        ...
        If Val(.TextMatrix(RowIndex, 11)) > 0 Then
            SubAmt = SubAmt + Val(.TextMatrix(RowIndex, 11))
        End If
        '--Tot Bag
    ...
    If FormAction = vbDataActionAddNew Then
        If txt(24) = 0 Then
            If Abs((CDbl(txt(24)) - (AssValue * CDbl(txt(23)) / 100))) > 1 Then
               TaxAmt = AssValue * Val(txt(23)) / 100
            Else
               TaxAmt = Val(txt(24))
            End If
        Else
