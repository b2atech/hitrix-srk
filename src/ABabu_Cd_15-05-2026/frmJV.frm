...
Case 0, 1:
    ...
    Do While Vno <= txtno(1)
        X = "Select * From tblVoucher where  vNo=" & Vno & " and VCTRNO =1 and VType='" & frmTypeStr & "' and vfirm = '" & gCCode & "'  and VYear=" & gCYear
        If GProcRstOpen(Rs1, X, "R") > 0 Then
            With frmMain.cryReport
                .SelectionFormula = "{tblVoucher.vNo} = " & Rs1!Vno & " and {tblVoucher.VCTRNO}=1 and {tblVoucher.VType}='" & Rs1!Vtype & "' and {tblVoucher.VFirm}='" & Rs1!VFirm & "'   and {tblVoucher.VYear}=" & Rs1!VYear
                .Formulas(5) = "wAmountInWords = '" & GProcAmountInWords(Abs(Rs1!vamt)) & "'"
                .Formulas(6) = "FirmGstin = 'GSTIN : " & gCGStin & "'"
...
With msGrid
    ...
    For i = 1 To .rows - 1
        TcsTdsCd = GProcGetColumnValue("tblMastAccount", "AcName", .TextMatrix(.Row, 2), "S", "Accode", "N")
        If TcsTdsCd = gTDSAcCodeRec Or TcsTdsCd = gTcsRec Then
           mIsTcsTdsAc = True
        Else
           mIsTcsTdsAc = False
        End If
        If mIsTcsTdsAc = True Then
          ChkAcName = GProcGetColumnValue("tblMastAccount", "AcOurCode", UCase(.TextMatrix(.Row, 6)), "S", "AcName", "S")
          If ChkAcName = "" Then
             MsgBox ("TDS/TCS Party Code Not found..")
             .Col = 6
             msGrid.SetFocus
             Exit Function
          End If
...
If FormAction = vbDataActionAddNew Then
    ...
    rstAddCheck.Close
End If
If FormAction = vbDataActionAddNew And gfrmTypeStr <> "JV" And txt(55) <> "" Then
    X = "Select CrDrNoteNo From " & tblName & " Where CrDrNoteNo='" & txt(55) & "' and Vtype='" & frmTypeStr & "' and VYear=" & gCYear
    i = GProcRstOpen(rstAddCheck, X, "R")
    If i > 0 Then
        MsgBox "Record is Available of No. " & txt(55), vbCritical + vbOKOnly, "Duplicate Record"
        txt(0).SetFocus
        Exit Function
    End If
    rstAddCheck.Close
...
If Index = 41 Then
   ...
   Dim Pcd As Long
   Pcd = GProcGetColumnValue("tblMastAccount", "AcName", txttemp(7), "S", "Accode", "N")

   X = "select SlBroker from tblSale where SlAcDrCode = " & Pcd & " and vtype = 'SY' order by vdt desc "
   If GProcRstOpen(tmpRec, X, "R", gCn) > 0 Then
      txttemp(41) = GProcGetColumnValue("tblMastAccount", "AcCode", tmpRec.Fields(0), "N", "AcName", "S")
   End If
End If
End Sub
...
Select Case Index
    Case 7: '--A/c Name
        ...
        If gClsSearch.SearchMultiRetColAccMaster(0) <> "" Then
            ...
            txtCode(7) = GProcGetColumnValue("tblMastAccount", "AcName", txttemp(7), "S", "AcOurCode", "S")
            txt(7) = GProcGetColumnValue("tblMastAccount", "AcName", txttemp(7), "S", "Accode", "N")
            If txt(7) = gTDSAcCodeRec Or txt(7) = gTcsRec Then
               mIsTcsTdsAc = True
            Else
               mIsTcsTdsAc = False
            End If
...
X = "select VCtrNo,tblMastAccount.AcOurCode,tblMastAccount.AcName,SubOurCode,NoInterest,VAmt,tblMastAccount_T.AcOurCode ,Vnar1,Vnar2" _
& " from tblVoucher,tblMastAccount,tblMastAccount tblMastAccount_T,tblMastSubGroup where " _
& " VType='" & frmTypeStr & "' and VNo=" & txt(0) & " and VFirm  = '" & gCCode & "' and  VYear=" & gCYear & " and " & TopCd & "  = tblMastAccount.AcCode and SubGroupCd = tblMastSubGroup.SubCode and TdsTcsPartyCd = tblMastAccount_T.AcCode  order by VCtrNo"
x1Rec = GProcRstOpen(rsRecVsSale, X, "R")
With msGrid
    .Clear
    .FormatString = ">Sr |<A/c Code |<Account Name                              |Sub Grp.|Int.|>Amount             |TDS/TCS Pty.|<Narration 1                      |<Narration 2                       "
    If FormAction = vbDataActionAddNew Then
        .rows = 2
        GridAddNew
    Else
...
With msGrid
    If KeyAscii = 27 Then '----Esc Key
        KeyAscii = 0
        If (Val(.TextMatrix(.Row, 5)) = 0) And .rows > 2 Then
            .RemoveItem (.Row)
            txtGrid.text = ""
...
'                    End If
   ...
   End If
   If KeyAscii = 13 Then
        If .Col = 2 And (.TextMatrix(.Row, 3) = "" Or .TextMatrix(.Row, 3) = "-") And mIsExpAc = True Then
           ...
           If i > 0 Then
            ...
            KeyAscii = 0
            If gClsSearch.SearchMultiRetCol(0) <> "" Then
               .TextMatrix(.Row, 3) = gClsSearch.SearchMultiRetCol(0)
               .Col = 4
            End If
          End If
        End If
        If .Col = 2 Then
           TcsTdsCd = GProcGetColumnValue("tblMastAccount", "AcName", .TextMatrix(.Row, 2), "S", "Accode", "N")
           If TcsTdsCd = gTDSAcCodeRec Or TcsTdsCd = gTcsRec Or TcsTdsCd = gTdsOnSalesCode Then
              mIsTcsTdsAc = True
           Else
              mIsTcsTdsAc = False
            End If
        End If
        If .Col = 6 And mIsTcsTdsAc = True Then
            txtGrid = UCase(txtGrid)
           .TextMatrix(.Row, 6) = txtGrid
           X = GProcGetColumnValue("tblMastAccount", "AcOurCode", UCase(.TextMatrix(.Row, 6)), "S", "AcName", "S")
           If X = "" Then
              txtGrid = ""
              .TextMatrix(.Row, 6) = ""
           Else
           ...
           End If
        End If
        If .Col = 6 And mIsTcsTdsAc = False Then
           .TextMatrix(.Row, 6) = ""
        End If
        CheckForNumberTxtGrid
    End If
    ...
    If KeyAscii = 13 And .Col < 8 Then
       If .Col = 5 And mIsTcsTdsAc = False Then
          .Col = .Col + 1
       End If
       .Col = .Col + 1
...
With msGrid
     If (.Col = 7 Or .Col = 8) And KeyCode = vbKeyF1 Then
...
With msGrid
    ...
    Do While i < .rows
        DoEvents
        If CDec(Val(.TextMatrix(i, 5))) > 0 Then
            rstbl.AddNew
            ...
            End If
            If (.TextMatrix(i, 3)) = "" Or IsNull((.TextMatrix(i, 3))) Then
               rstbl!SubGroupCd = 1
            Else
               rstbl!SubGroupCd = GProcGetColumnValue("tblMastSubGroup", "SubOurCode", .TextMatrix(i, 3), "S", "SubCode", "N")
            End If
            rstbl!NoInterest = .TextMatrix(i, 4) & vbNullString
            rstbl!vamt = CDec(.TextMatrix(i, 5))
            If (.TextMatrix(i, 6)) = "" Or IsNull((.TextMatrix(i, 6))) Then
               rstbl!TdsTcsPartyCd = 1
            Else
               rstbl!TdsTcsPartyCd = GProcGetColumnValue("tblMastAccount", "AcOurCode", .TextMatrix(i, 6), "S", "AcCode", "N")
            End If
            ...
            rstbl!VNar1 = .TextMatrix(i, 7) & vbNullString
            rstbl!VNar2 = .TextMatrix(i, 8) & vbNullString
    ...
    If Not .EOF Then
        ...
        Do While Not .EOF
            For ColIndex = 0 To .Fields.Count - 1
                ...
                End Select
                If ColIndex = 5 Then BnkTot = BnkTot + CLng(msGrid.TextMatrix(RowIndex, 5)) '-- Vamt
            Next ColIndex
            RowIndex = RowIndex + 1
            .MoveNext
...
With msGrid
    Do While RowIndex < .rows
        ...
        If Val(.TextMatrix(RowIndex, 5)) > 0 Then
            BnkTot = BnkTot + Val(.TextMatrix(RowIndex, 5))
        End If
        RowIndex = RowIndex + 1
    Loop
End With
...
Else
   lblCrDrNtNo.Visible = True
   txt(55).Visible = True
   cboCrDr.Enabled = False
End If
If FormAction = vbDataActionAddNew Then
   txt(0).Enabled = False
   txt(55).Enabled = False
