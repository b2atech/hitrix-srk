...
Begin VB.Form frmSalesOldOs 
   ...
   Begin VB.ComboBox Cbo 
      ...
      Height          =   420
      Index           =   44
      ItemData        =   "frmSalesOldOs.frx":058A
      Left            =   8865
      List            =   "frmSalesOldOs.frx":0594
      TabIndex        =   121
      Text            =   "Cbo(44)"
      Top             =   6165
      Visible         =   0   'False
   ...
   Begin Crystal.CrystalReport cryReport 
      Bindings        =   "frmSalesOldOs.frx":05A1
      Left            =   4110
      Top             =   0
      _ExtentX        =   741
      _ExtentY        =   741
   ...
   End
End
Attribute VB_Name = "frmSalesOldOs"
...
Case 4 'Find ' QryMastAccount
       gClsSearch.SearchMultiField "tblSale,TblMastAccount", "vno,SlBillNo,Vdt,AcName", Array("Vno", "Bill No", "Vdt", "Party"), Array(0, txt(9).Width, 1500, 4000), " tblSale.SlAcDrCode=TblMastAccount.AcCode and vtype='" & frmTypeStr & "'   and VFirm = '" & gCCode & "'   and VYear=" & gOldBillYear, "", "SlBillNo", 0, 0
'       gClsSearch.SearchMultiField "tblSale,TblMastAccount", "vno,SlBillNo,Vdt,AcName", Array("Vno", "Bill No", "Vdt", "Party"), Array(0, txt(9).Width, 1500, 4000), " tblSale.SlAcDrCode=TblMastAccount.AcCode and vtype='" & frmTypeStr & "'   and VFirm = '" & gCCode & "'   and VYear=" & gOldBillYear, "", "(cast(substring(SlBillNo,5,15) as int) ) desc", 0, 0
...
Case 12 'Optional
        ...
        End If
Case 13 'Save
     '-for broker and duedaye change
     
     FillTxtFromTemp
'     ReadFields
     
     gCn.Execute "update tblsale set SlBroker = " & Val(txt(8)) & ",DueDays = " & Val(txt(31)) & " " _
                               & " where  tblsale.Vno = " & Val(txt(0)) & " and  tblsale.Vtype =  '" & txt(1) & "'  and tblsale.vyear = " & Val(txt(3)) & "   and tblsale.Vfirm = '" & txt(4) & "' "
     
     gCn.Execute "update tblIntSale set Broker = " & Val(txt(8)) & " " _
                               & " where  tblIntSale.VBillVno = " & Val(txt(0)) & " and  tblIntSale.VBillType =  '" & txt(1) & "'  and tblIntSale.VBillVYear = " & Val(txt(3)) & "   and tblIntSale.VBillFirm = '" & txt(4) & "' "
     GProcRstOpen rstbl, "Select * from " & tblName & " where " & IdField & " = " & txt(0) & " and " & Criteria, "O"
...
'''               Mon = Month(dtpDt(2))
'''               txt(0) = GProcGenerateId("TblSale", "Vno", "Vtype='" & frmTypeStr & "' and VFirm = '" & gCCode & "'   and VYear=" & gOldBillYear)
'''            End If
...
Case 0, 1:
    ...
    X = "Select a.Vno,a.Vtype,a.Vdt,a.Vyear,a.VFirm,a.SlAcDrCode,a.SlAcCrCode,a.SlMillCode,a.SlBroker,a.SlBillNo,a.SlBillDt,a.SlTransport,a.SlLorryNo,a.SlLrNo,a.SlLrDate, " _
        ...
        & " b.SlSubBookNo,b.SlSubBookDt,b.SlSubGodown,b.SlSubLotNo from tblSale A ,tblSaleSub B" _
        & " where a.vno = b.vno and a.vtype = b.vtype and a.vyear = b.vyear and a.vfirm = b.vfirm and  a.SlBillNo in " & X & " and a.Vtype='" & frmTypeStr & "' and a.VFirm = '" & gCCode & "' And a.VYear = " & gOldBillYear
   ...
   gCn.Execute " insert into tmpSale (Vno,Vtype,Vdt,Vyear,VFirm,SlAcDrCode,SlAcCrCode,SlMillCode,SlBroker,SlBillNo,SlBillDt,SlTransport,SlLorryNo,SlLrNo,SlLrDate, " _
               ...
               & " SlSubBookNo,SlSubBookDt,SlSubGodown,SlTmpChr1) " & X
     With cryReport
...
Private Sub Form_Load()
    ...
    If frmTypeStr = "SM" Then
        LblSaleType.Caption = "   Mill Bill ( Direct - Sale)"
        TrnCd = gTranCdSO
    ElseIf frmTypeStr = "SD" Then ' --- Depot Sale
        LblSaleType.Caption = "   Depot Sale/Consignment Sale"
        TrnCd = gTranCdSD
    ElseIf frmTypeStr = "SY" Then
           SalesAccode = gAcCodeSY
           TrnCd = gTranCdSY
          LblSaleType.Caption = "   Trade - SIT- Exemption Sale"
    ...
    txttemp(6) = GProcGetColumnValue("tblMastAccount", "AcCode", CStr(SalesAccode), "N", "AcName", "S")
    FirstTimeFlag = True
    Criteria = "Vtype='" & frmTypeStr & "' and VFirm  = '" & gCCode & "' and VYear=" & gOldBillYear & "  "
...
& " and tblSaleSub.vtype='" & frmTypeStr & "' and tblSaleSub.vno=" & txt(0) & " and tblSaleSub.VFirm = '" & gCCode & "' and tblSaleSub.VYear=" & gOldBillYear & " order by SlSubItSrNo"
...
If Index = 2 Then
    If GProcIsDateValidOldOs(dtpDt(Index)) = False Then
...
If Index = 2 Then
   ...
   If FormAction = vbDataActionAddNew Then
        ...
        If Val(txtSlSub(17)) = 0 Then
            If GProcRstOpen(Rs1, "Select SlSubItCode,SlSubRt from tblSale,tblSaleSub where SlMillCode = " & MilCd & "  and tblsale.vtype = '" & frmTypeStr & "' and tblsale.Vfirm = '" & gCCode & "' and tblsale.vyear = " & gOldBillYear & " and SlBillNo= '" & LastBillNo & "' and tblSale.vno = tblSaleSub.vno and tblSale.vtype = tblSaleSub.vtype and tblSale.vyear = tblSaleSub.vyear and tblSale.vfirm = tblSaleSub.vfirm", "R") > 0 Then
...
If Index = 9 And FormAction <> vbDataActionCancel Then
   ...
   X = "Select SlBillNo From " & tblName & " Where SlBillNo = '" & txt(9) & "'  and Vtype='" & frmTypeStr & "'  and Vfirm = '" & gCCode & "' and  VYear=" & gOldBillYear
...
'             End If
             ShowMill = "AgCode=90029 and   tblMastaccount.Accode = tblMastBillSerial.MillCode "
...
'                    gClsSearch.SearchMultiField "tblMastItem,tblMastaccount", "ItName,AcName,ItStdpack,ItStdrateper", Array("Count Name", "Mill", "Std. Pack", "Rate Per"), Array(txtGrid.Width, 2000, 1000, 1000), "tblMastItem.ITMillCode=tblMastaccount.acCode and ITMillCode=" & MillCode, Chr(KeyAscii), "ItName", LeftPos, TopPos, True
                    ...
                    If frmTypeStr = "SO" Or frmTypeStr = "SD" Then
                        If gClsSearch.SearchMultiRetCol(0) <> "" Then '-- Item Wt ,Rate From Master
                              ...
                              BalBag = BalBag - GProcGetColumnValue("tblSaleSub", "SlSubItCode", Val(gClsSearch.SearchMultiRetCol(4)), "N", "sum(SlSubBag)", "N", "VType = '" & frmTypeStr & "' and VFirm = '" & gCCode & "' and Vdt<=('" & Format(dtpDt(2), "YYYY/MM/DD") & "') and  not (vno = " & txt(0) & " and VYear=" & gOldBillYear & " and VFirm = '" & gCCode & "'  and vtype = '" & frmTypeStr & "')")
                              ...
                              BalWt = BalWt - GProcGetColumnValue("tblSaleSub", "SlSubItCode", Val(gClsSearch.SearchMultiRetCol(4)), "N", "sum(SlSubWt)", "N", "VType = '" & frmTypeStr & "' and VFirm = '" & gCCode & "' and Vdt<=('" & Format(dtpDt(2), "YYYY/MM/DD") & "') and  not (vno = " & txt(0) & " and VYear=" & gOldBillYear & " and VFirm = '" & gCCode & "'  and vtype = '" & frmTypeStr & "')")
...
If (CDbl(txt(0)) = 0 And FormAction <> vbDataActionAddNew) Or GProcIsDateValidOldOs(dtpDt(2)) = False Or txttemp(5) = "" Or txttemp(6) = "" Or txttemp(7) = "" Or CDbl(txt(16)) = 0 Or CDbl(txt(15)) = 0 Then
    cmdBtn(13).Enabled = False
...
'    End If
    ...
    End If
    '--- Vdt
    If GProcIsDateValidOldOs(dtpDt(2)) = False Then
    ...
    End If
    If Trim(txt(38)) <> "" Then
        X = "Select SlBillNo From " & tblName & " Where GpNo= '" & txt(38) & "' and   vno <> " & CLng(txt(0)) & "  and Vtype='" & frmTypeStr & "'  and Vfirm = '" & gCCode & "'  and VYear=" & gOldBillYear
        i = GProcRstOpen(rstAddCheck, X, "R")
        If i > 0 Then
...
'            txt(9).SetFocus
 '           Exit Function
    ...
    '----Addition Case
    If FormAction = vbDataActionAddNew Then
        X = "Select vno From " & tblName & " Where vno=" & CLng(txt(0)) & " and Vtype='" & frmTypeStr & "'  and Vfirm = '" & gCCode & "'  and VYear=" & gOldBillYear
        i = GProcRstOpen(rstAddCheck, X, "R")
        If i > 0 Then
...
If IsDate(dtpDt(2)) Then '--Vdt
    txt(2) = CDate(dtpDt(2))
    txt(10) = CDate(dtpDt(2))
    txt(12) = CDate(dtpDt(2))
    txt(18) = CDate(dtpDt(2))
Else
    txt(2) = ""
End If
...
& "  VBillVno = " & txt(0) & " and VFirm  = '" & gCCode & "' and  VBillVYear= " & gOldBillYear & ""
GProcRstOpen RsG, X, "R"
If RsG.EOF Then
Else
    IsRelatedRecord = True
...
If gClsSearch.SearchMultiRetCol(0) <> "" Then
   ...
   X = "Select * from tblGatePassSub where VNo=" & TmpGpVno & " and vtype = 'GP' and VFirm  = '" & gCCode & "' and VYear= " & gOldBillYear & " and GpSubGpType=" & GPSLType & "  "
