...
Case 0, 1:
    ...
    frmMain.cryReport.WindowTitle = "RTGS Form Printing"
    
    X = "select tblVoucher.Vno,tblVoucher.VCtrNo,tblMastAccount_Bnk.AcPartyBank,tblMastAccount_Bnk.AcPartyAcNo, " _
        & " tblVoucher.VrefTp,tblVoucher.VrefNo,tblVoucher.VRefDate,tblMastAccount_Pty.AcName,tblMastAccount_Pty.AcPartyBank, " _
        & " tblMastAccount_Pty.AcPartyBranch , tblMastAccount_Pty.AcPartyIFSC, tblVoucher.Vdt, tblMastAccount_Pty.AcPartyAcNo, vamt,'" & gUserName & "' " _
        & " from tblVoucher,tblMastAccount tblMastAccount_Bnk, tblMastAccount tblMastAccount_Pty " _
        & " where vfirm = '" & gCCode & "' and vtype = 'BP' and VYear = " & gCYear & " and vno >= " & txtno(0) & " and vno >= " & txtno(1) & " and " _
        & " tblVoucher.VCrAcCode = tblMastAccount_Bnk.AcCode And tblVoucher.VDrAcCode = tblMastAccount_Pty.AcCode "
   gCn.BeginTrans
   gCn.Execute "Delete from TmpGentbl where  UserName='" & gUserName & "'"
     
   gCn.Execute " insert into TmpGentbl (Vno,VSrNo,Nar1,Nar2,Nar3,Nar4,Dt1,Nar5,Nar6,Nar7,Nar8,Vdt,Nar9,Amt1,UserName) " & X
   gCn.CommitTrans
...
'   With cryReport
        ...
        .SelectionFormula = X
        .ReportFileName = gReportPath & "rptRTGS_Half_PagePrint.rpt"
        .Action = 1
   End With
Case 2:
    DoEvents
...
X = "select VCtrNo,AcOurCode,AcName, VAmt,left(VRefTp,1),VRefTp,VRefNo,SubOurCode,NoInterest,VRefBank,VBillNo,VBillType,VBillDate,VBillAmt,VBillVYear,VBillVno,VBillFirm" _
& " from tblVoucher,tblMastAccount,tblMastSubGroup where " _
& " VType='" & frmTypeStr & "' and VNo=" & txt(0) & " and VFirm  = '" & gCCode & "' and  VYear=" & gCYear & " and VDrAcCode = tblMastAccount.AcCode and SubGroupCd = tblMastSubGroup.SubCode order by VCtrNo"
x1Rec = GProcRstOpen(rsRecVsSale, X, "R")
With msGrid
    .Clear
    .FormatString = ">Sr|<Party Code|<Party Name                                  |>Paid. Amt     |   |<Ref  |<Ref.No.     |Sub Grp.|Int.|<Narration                   |<Bill No           |<Tp  ||>Bill Amt          |||"
    .ColWidth(12) = 0
    .ColWidth(14) = 0
    .ColWidth(15) = 0
    .ColWidth(16) = 0
...
With msGrid
    ...
    If KeyAscii = 13 Then
        KeyAscii = 0
        If .Col = rsRecVsSale.Fields.Count - 5 Then '-4
        ...
        If frmTypeStr = "BP" Then
            ...
            If .Row > 1 And FormAction = vbDataActionAddNew Then
               If msGrid.TextMatrix(.Row, 2) = msGrid.TextMatrix(.Row - 1, 2) Then
                  msGrid.TextMatrix(.Row, 9) = msGrid.TextMatrix(.Row - 1, 9)
               End If
            End If
        End If
    End With
...
Case 1, 2:
    With msGrid
        If .TextMatrix(.Row, 1) <> "" Then
           ...
           GpCd = GProcGetColumnValue("tblMastAccount", "AcOurCode", UCase(.TextMatrix(.Row, 1)), "S", "AgCode", "N")
           If GpCd = 90028 Then
              mIsExpAc = True
           Else
              mIsExpAc = False
        ...
        If .TextMatrix(.Row, 4) = "" Then
           ...
           End If
        End If
        If .TextMatrix(.Row, 9) = "" And .TextMatrix(.Row, 1) <> "" Then
           j = GProcGetColumnValue("tblMastAccount", "AcOurCode", "" & .TextMatrix(.Row, 1) & "", "S", "AcCode", "N")
           DoEvents
           .TextMatrix(.Row, 9) = GProcGetColumnValue("tblVoucher", "VDrAcCode", "" & j & "", "N", "VRefBank", "S", "Vtype = '" & frmTypeStr & "' and Vfirm = '" & gCCode & "' order by vdt desc,vno desc, VCtrNo desc  ")
        End If
    End With
Case 9:
    With msGrid
         .TextMatrix(.Row, 9) = GProcProperCase(.TextMatrix(.Row, 9))
    End With
End Select
CalBalance
End Sub
...
With msGrid
        ...
        Select Case .Col
            Case 2 '-- party
                    ...
                    If gClsSearch.SearchMultiRetColAccMaster(0) <> "" Then
                        ...
                        GpCd = GProcGetColumnValue("tblMastAccount", "AcName", "" & gClsSearch.SearchMultiRetColAccMaster(0) & "", "S", "AgCode", "N")
                        If GpCd = 90028 Then
                           mIsExpAc = True
                        Else
                           mIsExpAc = False
   ...
   If KeyAscii = 13 Then
        CheckForNumberTxtGrid
        If .Col = 6 And (.TextMatrix(.Row, 7) = "" Or .TextMatrix(.Row, 7) = "-") And mIsExpAc = True Then
           GpCd = GProcGetColumnValue("tblMastAccount", "AcName", "" & .TextMatrix(.Row, 2) & "", "S", "AcCode", "N")
           ...
           If i > 0 Then
            ...
            KeyAscii = 0
            If gClsSearch.SearchMultiRetCol(0) <> "" Then
               .TextMatrix(.Row, 7) = gClsSearch.SearchMultiRetCol(0)
'               .TextMatrix(.Row, 9) = gClsSearch.SearchMultiRetCol(1)
               .Col = 7
            End If
          End If
        End If
        If (.Col = 10 And txtGrid <> "") Then
           Dim j As Integer
               j = .Row
               txtGrid = UCase(txtGrid)
               If .Col = 10 Then
                  .TextMatrix(j, 10) = UCase(txtGrid)
                  X = "select tblMastAccount.AcOurCode,tblMastAccount.AcName,SlBillNo,VType,format(Vdt,'dd/MM/yyyy'),SlBillAmt,SlBillAmt-SlBillRecdAmt," _
                      ...
                      & " SlBillNo = '" & .TextMatrix(j, 10) & "' and vfirm = '" & gCCode & "'"
               End If
'               If .Col = 4 Then
...
'               End If
                ...
                End If
                If i > 0 Then
                   .TextMatrix(j, 10) = rstblBlChk.Fields(2)  '--Bill No
                   .TextMatrix(j, 11) = rstblBlChk.Fields(3)  '--Type
                   .TextMatrix(j, 12) = Format(rstblBlChk.Fields(4), "dd/MM/yyyy") '--VDt
                   .TextMatrix(j, 1) = rstblBlChk.Fields(0) & vbNullString '--Party Code
                   .TextMatrix(j, 2) = rstblBlChk.Fields(1) '--Party
                   .TextMatrix(j, 13) = Format(rstblBlChk.Fields(5), "#######0.00") '--Bill amt
'                   If FormAction = vbDataActionAddNew Then
 '                     .TextMatrix(J, 3) = Format(rstblBlChk.Fields(6), "#######0.00") '--Balance Amt
  '                 End If
                   .TextMatrix(j, 14) = CLng(rstblBlChk.Fields(7)) '--Year
                   .TextMatrix(j, 15) = rstblBlChk.Fields(8) '--Vno
                   .TextMatrix(j, 16) = rstblBlChk.Fields(9) '--vBillFirm
...
With msGrid
     If .Col = 9 And KeyCode = vbKeyF1 Then
     ...
     End If
     If .Col = 10 And KeyCode = vbKeyF1 Then
         X = "select tblMastAccount.AcOurCode,tblMastAccount.AcName,SlBillNo,VType,format(Vdt,'dd/MM/yyyy'),SlBillAmt,SlBillAmt-SlBillRecdAmt," _
             & " VYear,Vno,ltrim(str(Vno))+'-'+Vtype+'-'+ltrim(str(Vyear))+'-'+rtrim(vfirm) as VnoVtypeVyearVfirm,tblMastAccount.AcCode from tblSale, tblMastAccount where tblSale.SlAcDrCode = tblMastAccount.AcCode and " _
             & " SlBillAmt-SlBillRecdAmt > 0 and vfirm = '" & gCCode & "'"
         i = GProcRstOpen(rstblBlChk, X, "R")
...
With msGrid
    ...
    Do While i < .rows
        ...
        If CDec(Val(.TextMatrix(i, 3))) > 0 Then
            ...
            rstbl!VBillNo = .TextMatrix(i, 10) & vbNullString
            rstbl!VBillType = .TextMatrix(i, 11) & vbNullString
            If IsDate(.TextMatrix(i, 12)) Then
               rstbl!VBillDate = CDate(.TextMatrix(i, 12))
            End If
            rstbl!VBillAmt = Val(.TextMatrix(i, 13))
            rstbl!vamt = CDec(.TextMatrix(i, 3))
            rstbl!VRefTp = .TextMatrix(i, 5) & vbNullString
            rstbl!VRefNo = .TextMatrix(i, 6) & vbNullString
            If (.TextMatrix(i, 7)) = "" Or IsNull((.TextMatrix(i, 7))) Then
               rstbl!SubGroupCd = 1
            Else
               rstbl!SubGroupCd = GProcGetColumnValue("tblMastSubGroup", "SubOurCode", .TextMatrix(i, 7), "S", "SubCode", "N")
            End If
            rstbl!NoInterest = .TextMatrix(i, 8) & vbNullString
            rstbl!VRefBank = .TextMatrix(i, 9) & vbNullString
            rstbl!VBillVYear = .TextMatrix(i, 14)
            rstbl!VBillVno = .TextMatrix(i, 15)
            rstbl!VBillFirm = .TextMatrix(i, 16)
            rstbl!VNar1 = ""
            rstbl!VNar2 = ""
            rstbl!VNar3 = ""
            rstbl!VNAR4 = ""
