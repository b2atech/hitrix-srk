...
'            If frmTypeStr = "SD" And Index = 7 And txttemp(7) = "" Then  ' --- Depot Sale
'               Call txtTemp_KeyPress(7, 32)
'            End If
...
'     i = InStr(1, txt(9), "/")
'     if
'CONVERT(numeric, STR(SUBSTRING(SlBillNo,5,15),7,2) ) as a_number
    gClsSearch.SearchMultiField "tblSale,TblMastAccount", "vno,SlBillNo,Vdt,AcName", Array("Vno", "Bill No", "Vdt", "Party"), Array(0, txt(9).Width, 1500, 4000), " tblSale.SlAcDrCode=TblMastAccount.AcCode and vtype='" & frmTypeStr & "'  and SlIsHank = 0  and VFirm = '" & gCCode & "'   and VYear=" & gCYear, "", "(cast(substring(SlBillNo,5,15) as int) ) desc", 0, 0
...
Case 6 'Add
    ...
    SetControlEd
'    If frmTypeStr = "SD" Then ' --- Depot Sale
'        X = GProcGetColumnValue("tblMastAccount", "AcDepotFirm", "" & gCCode & "", "S", "AcDepotFirm", "S")
'        If X = gCCode Then
'           X = GProcGetColumnValue("tblMastAccount", "AcDepotFirm", "" & gCCode & "", "S", "Acname", "S")
'           txttemp(7) = X
'           X = GProcGetColumnValue("tblMastAccount", "AcDepotFirm", "" & gCCode & "", "S", "AcOurCode", "S")
'           txtCode(7) = X
'           SalesAccode = GProcGetColumnValue("tblMastAccount", "AcDepotFirm", "" & gCCode & "", "S", "AcCode", "S")
...
'End If
'If frmTypeStr = "SD" Or frmTypeStr = "SO" Then
'   txttemp(7).Enabled = False
'   txtCode(7).Enabled = False
'End If
End Sub
...
& " ,SlSubRt,SlSubRtPer,SlSubNetRate,SlSubAmt,SlSubItCtrlNo,SlSubPItCtrlNo from tblSaleSub,TblMastNarration,TblMastItem where " _
...
If Index = 2 Then
    ...
    If dtpDt(2) >= CDate("01/07/2017") Then
       MsgBox "Date in GST Applicabel " & txt(9), vbCritical + vbOKOnly, ""
       dtpDt(Index).SetFocus
       Exit Sub
    End If
    If dtpDt(2) >= CDate("01/07/2017") Then
        Label21.Caption = "GSTIN :"
...
If msGrid.Col < rstblSub.Fields.Count - 1 Then
   ...
   If msGrid.Col = 4 Or msGrid.Col = 5 Then
      ...
      If frmTypeStr = "SE" Or frmTypeStr = "SY" Then
           ...
           QStr = "Select tblPurch.PurBillNo,format(tblPurchSub.Vdt,'dd/MM/yyyy'),tblPurchSub.PurSubBag-tblPurchSub.PurSubIssBag+ " & ModiBags & ",tblPurchSub.PurSubWt-tblPurchSub.PurSubIssWt+" & ModiWt & ",tblPurchSub.PurSubItCtrlNo,tblPurchSub.Vno " _
                  ...
                  & " tblPurchSub.VFirm = '" & gCCode & "'  and tblPurchSub.vtype = '" & PurTp & "' and tblpurch.vno = tblPurchSub.vno and tblpurch.vtype = tblPurchSub.vtype and tblpurch.vyear = tblPurchSub.vyear and tblpurch.vfirm = tblPurchSub.vfirm  "
...
'                    gClsSearch.SearchMultiField "tblMastItem,tblMastaccount", "ItName,AcName,ItStdpack,ItStdrateper", Array("Count Name", "Mill", "Std. Pack", "Rate Per"), Array(txtGrid.Width, 2000, 1000, 1000), "tblMastItem.ITMillCode=tblMastaccount.acCode and ITMillCode=" & MillCode, Chr(KeyAscii), "ItName", LeftPos, TopPos, True
                    ...
                    If gClsSearch.SearchMultiRetCol(0) <> "" Then '-- Item Wt ,Rate From Master
                        ...
                        txtGrid_KeyPress 13
                    End If
                    If frmTypeStr = "SO" Or frmTypeStr = "SD" Or frmTypeStr = "ST" Then
...
Select Case frmTypeStr    '-- Tax Type
    ...
    Case "SE"
         Cbo(22) = GProcGetColumnValue("tblMastNarration", "NarrCode", CStr(gTaxCodeSYExempt), "N", "Narration", "S")
    Case "ST"
...
txt(46) = GProcGetColumnValue("tblMastAccount", "AcName", txttemp(46), "S", "AcCode", "N")
...
'--Taxpayable Ac
txttemp(46) = GProcGetColumnValue("tblMastAccount", "AcCode", txt(46), "N", "AcName", "S")
...
With msGrid
    Do While RowIndex < .rows
        ...
        If RowIndex = 1 And msGrid.TextMatrix(1, 3) <> "" And dtpDt(2) < CDate("01/07/2017") Then
           '-- 0 - Cotton
           '-- 1 - Polyster
...
txt(16) = Round(Amt2, 0)
CheckForNumber
TaxCd = GProcGetColumnValue("tblMastNarration", "Narration", Cbo(22).text, "S", "MastTaxSAAcCode", "N")
txttemp(46) = GProcGetColumnValue("tblMastAccount", "AcCode", CStr(TaxCd), "N", "AcName", "S")
