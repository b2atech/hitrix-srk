...
Begin VB.Form frmLatePaymentIntGST 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00FBF2E1&
   BorderStyle     =   0  'None
   ...
   EndProperty
   Icon            =   "frmLatePaymentIntGST.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Frame2 
         ...
         End
      End
      Begin VB.CheckBox chkIsNonGSTDbNote 
         BackColor       =   &H00FFC0C0&
         Caption         =   "Non GST Debit Note"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   9.75
            Charset         =   0
         ...
         Width           =   2895
      End
      Begin VB.CheckBox chkIsGSTDbNote 
         BackColor       =   &H00FFC0C0&
   ...
   Begin Crystal.CrystalReport cryReport 
      Bindings        =   "frmLatePaymentIntGST.frx":058A
      Left            =   -45
      Top             =   660
      _ExtentX        =   741
      _ExtentY        =   741
   ...
   End
End
Attribute VB_Name = "frmLatePaymentIntGST"
...
Case 6 'Add
    ...
    CondNewVno = " VType = 'BR' and  VnoList <> 0 and VFirm in (" & BillsInFirm & ") and VNoListVYear=" & gCYear
    txt(35).text = GProcGenerateId(tblName, "VnoList", CondNewVno, gCn)
...
'           gCn.Execute "Delete from tblvoucher where " & CriteriaStr & " and vno = " & txt(0)
           ...
           If gCYear >= 2023 And gCIsLog = 1 Then
              gCn.BeginTrans
              XX = "Select max(LogNo) from tblIntSale_Log where VType in ('SI','MI') and VFirm = '" & gCCode & "'  and VYear = " & Val(txt(3)) & " and  vnolist  = " & txt(35)
              ...
              Else
                 wLogNo = 1
              End If
              gCn.Execute "Insert into tblIntSale_Log select tblIntSale.*," & wLogNo & " as LogNo,'D' as LogTp ,'" & gUserName & "' as UserName ,'" & Format(Now, "YYYY/MM/DD hh:mm:ss") & "' as LogDate from tblIntSale where VType in ('SI','MI') and VFirm = '" & gCCode & "'  and VYear = " & Val(txt(3)) & " and  vnolist  = " & txt(35)
              gCn.CommitTrans
              tmpRst.Close
           End If
           ...
           If Not RsP.EOF Then
              ...
              With RsP2
                If Not .EOF Then
                   ...
                   Do While Not .EOF
                      gCn.BeginTrans
                      XX = "Select max(LogNo) from tblvoucher_Log where VType = 'JV' and VFirm = '" & gCCode & "'  and VYear = " & Val(txt(3)) & "  and VCtrNo = " & RsP2.Fields(0) & "   and  vno = " & JvNoTDs
                      jj = GProcRstOpen(tmpRst, XX, "R")
                      If IIf(IsNull(tmpRst.Fields(0)), 0, tmpRst.Fields(0)) > 0 Then
                         wLogNo = IIf(IsNull(tmpRst.Fields(0)), 0, tmpRst.Fields(0)) + 1
                      Else
                         wLogNo = 1
                      End If
                      gCn.Execute "Insert into tblvoucher_Log select tblvoucher.*," & wLogNo & " as LogNo,'D' as LogTp ,'" & gUserName & "' as UserName ,'" & Format(Now, "YYYY/MM/DD hh:mm:ss") & "' as LogDate from tblvoucher where VType = 'JV' and VFirm = '" & gCCode & "'  and VYear = " & Val(txt(3)) & "  and VCtrNo = " & RsP2.Fields(0) & "   and  vno = " & JvNoTDs
                      gCn.CommitTrans
                      .MoveNext
                  Loop
                End If
...
Case 9 'Print
    ...
    chkIsGSTDbNote.Enabled = True
    chkIsNonGSTDbNote.Enabled = True
    txtno(0) = txt(35)
    txtno(1) = txt(35)
    txtno(0).SetFocus
Case 10 'Exit
...
'            & " tblVoucher.VType='" & frmTypeStr & "' and VnoList=" & Vno & " and tblVoucher.VFirm in (" & BillsInFirm & ") and  tblVoucher.VNoListVYear=" & gCYear & " and VCrAcCode = tblMastAccount.AcCode and  tblSale.SlBroker = tblMastAccount_Br.AcCode and tblVoucher.VFirm = dbo.tblMastCompany.CCode  and " _
'            & " VBillVno = tblSale.Vno and VBillType = tblSale.Vtype and VBillVYear = tblSale.Vyear and tblVoucher.VFirm = tblSale.Vfirm  and tblSale.Vno = tblSaleSub.vno  and tblSale.Vtype = tblSaleSub.vtype and tblSale.Vyear = tblSaleSub.vyear and tblsale.VFirm = tblSalesub.Vfirm and tblsalesub.SlSubItCode = tblMastItem.ItCode and tblsalesub.SlSubItSrNo = 1 "
        If OptType(0).Value = True Then
           If gCYear >= 2021 Then '--For ItHsn2,ItHsn
        ...
        Else
           If gCYear >= 2021 Then '--For ItHsn2,ItHsn
                X = "select tblVoucher.VFirm,tblMastCompany.Cname,tblVoucher.VBillNo,tblVoucher.VBillType, tblVoucher.VBillDate,tblVoucher.VBillAmt,tblMastAccount.AcOurCode,tblMastAccount.AcName,tblMastAccount.AcAdd1,tblMastAccount.AcAdd2,tblMastAccount.AcAdd3,tblMastAccount.AcGSTIN,tblMastAccount_Br.AcOurCode,tblMastAccount_Br.AcName, tblVoucher.VAmt,DueOnDt,tblVoucher.VRefDate,tblVoucher.LateDays,tblVoucher.Interest,tblVoucher.JvNoInt,tblVoucher.Grace,tblVoucher.IntRt,tblVoucher.DbNtMonthly,tblVoucher.DbNtInBillNo,tblVoucher.CgstRt,tblVoucher.SgstRt,tblVoucher.IgstRt,tblVoucher.CgstAmt,tblVoucher.SgstAmt,tblVoucher.IgstAmt,ItHsn2,tblVoucher.CrDrNoteNo,'" & gUserName & "'," _
                    & " tblVoucher.Vno,tblVoucher.Vdt,tblMastAccount.AcContNo,tblIntSale.SlIRNNo,tblIntSale.SlAckNo from tblVoucher,tblMastAccount,tblMastAccount tblMastAccount_Br,tblSale,tblSaleSub,tblMastItem,tblMastCompany,tblIntSale where " _
                    & " tblVoucher.VType='" & frmTypeStr & "' and tblVoucher.VnoList=" & Vno & " and tblVoucher.VFirm in (" & BillsInFirm & ") and  tblVoucher.VNoListVYear=" & gCYear & " and tblVoucher.VCrAcCode = tblMastAccount.AcCode and  tblSale.SlBroker = tblMastAccount_Br.AcCode and tblVoucher.VFirm = dbo.tblMastCompany.CCode  and " _
                    & " tblVoucher.VBillVno = tblSale.Vno and tblVoucher.VBillType = tblSale.Vtype and tblVoucher.VBillVYear = tblSale.Vyear and tblVoucher.VFirm = tblSale.Vfirm  and tblSale.Vno = tblSaleSub.vno  and tblSale.Vtype = tblSaleSub.vtype and tblSale.Vyear = tblSaleSub.vyear and tblsale.VFirm = tblSalesub.Vfirm and tblsalesub.SlSubItCode = tblMastItem.ItCode and tblsalesub.SlSubItSrNo = 1 " _
                    & " and tblVoucher.VnoList = tblIntSale.VnoList and tblVoucher.VNoListVYear = tblIntSale.VYear and tblVoucher.CrDrNoteNo = tblIntSale.CrDrNoteNo and tblVoucher.JvNoInt = tblIntSale.VNo and tblVoucher.VCrAcCode = tblIntSale.VDrAcCode"
           Else
                X = "select tblVoucher.VFirm,tblMastCompany.Cname,tblVoucher.VBillNo,tblVoucher.VBillType, tblVoucher.VBillDate,tblVoucher.VBillAmt,tblMastAccount.AcOurCode,tblMastAccount.AcName,tblMastAccount.AcAdd1,tblMastAccount.AcAdd2,tblMastAccount.AcAdd3,tblMastAccount.AcGSTIN,tblMastAccount_Br.AcOurCode,tblMastAccount_Br.AcName, tblVoucher.VAmt,DueOnDt,tblVoucher.VRefDate,tblVoucher.LateDays,tblVoucher.Interest,tblVoucher.JvNoInt,tblVoucher.Grace,tblVoucher.IntRt,tblVoucher.DbNtMonthly,tblVoucher.DbNtInBillNo,tblVoucher.CgstRt,tblVoucher.SgstRt,tblVoucher.IgstRt,tblVoucher.CgstAmt,tblVoucher.SgstAmt,tblVoucher.IgstAmt,ItHsn,tblVoucher.CrDrNoteNo,'" & gUserName & "'," _
                    & " tblVoucher.Vno,tblVoucher.Vdt,tblMastAccount.AcContNo,tblIntSale.SlIRNNo,tblIntSale.SlAckNo from tblVoucher,tblMastAccount,tblMastAccount tblMastAccount_Br,tblSale,tblSaleSub,tblMastItem,tblMastCompany,tblIntSale where " _
                    & " tblVoucher.VType='" & frmTypeStr & "' and tblVoucher.VnoList=" & Vno & " and tblVoucher.VFirm in (" & BillsInFirm & ") and  tblVoucher.VNoListVYear=" & gCYear & " and tblVoucher.VCrAcCode = tblMastAccount.AcCode and  tblSale.SlBroker = tblMastAccount_Br.AcCode and tblVoucher.VFirm = dbo.tblMastCompany.CCode  and " _
                    & " tblVoucher.VBillVno = tblSale.Vno and tblVoucher.VBillType = tblSale.Vtype and tblVoucher.VBillVYear = tblSale.Vyear and tblVoucher.VFirm = tblSale.Vfirm  and tblSale.Vno = tblSaleSub.vno  and tblSale.Vtype = tblSaleSub.vtype and tblSale.Vyear = tblSaleSub.vyear and tblsale.VFirm = tblSalesub.Vfirm and tblsalesub.SlSubItCode = tblMastItem.ItCode and tblsalesub.SlSubItSrNo = 1 " _
                    & " and tblVoucher.VnoList = tblIntSale.VnoList and tblVoucher.VNoListVYear = tblIntSale.VYear and tblVoucher.CrDrNoteNo = tblIntSale.CrDrNoteNo and tblVoucher.JvNoInt = tblIntSale.VNo and tblVoucher.VCrAcCode = tblIntSale.VDrAcCode"
           End If
        End If
            If chkIsGSTDbNote.Value = 1 And OptType(1).Value = True Then
               X = X + " and tblVoucher.CgstAmt + tblVoucher.SgstAmt + tblVoucher.IgstAmt > 0"
            End If
            If chkIsNonGSTDbNote.Value = 1 And OptType(1).Value = True Then
               X = X + " and tblVoucher.CgstAmt + tblVoucher.SgstAmt + tblVoucher.IgstAmt = 0"
            End If
'            X = X + " order by tblMastAccount.AcName,tblMastAccount_Br.AcName,tblVoucher.Vno,VCtrNo"
            X = X + " order by tblMastAccount.AcName,tblVoucher.Vno,tblVoucher.VCtrNo"
            gCn.Execute "Delete from TmpGentbl where  UserName='" & gUserName & "'"
     
            gCn.Execute " insert into TmpGentbl (VFirm,VFirmName,VBillNo,VBillType,VBillDt,Amt1,PartyCode,PartyName,Nar1,Nar4,Nar6,Nar5,BrokerCode,BrokerName,Amt2,Dt1,Vdt,Amt3,Amt4,Amt5,Amt6,amt7,amt8,Nar2,Amt9,Amt10,Amt11,Amt12,Amt13,Amt14,Nar3,Nar7,UserName,Amt15,Dt2,Nar8,nar9,nar10) " & X
'            X1 = "{TmpGentbl.UserName}='" & gUserName & "' and {@IntAmt} > 0  "
            With frmMain.cryReport
              ...
              If OptType(0).Value = True Then
                 .ReportFileName = gReportPath & "rptIntDbNtListGST.rpt"
              Else
                 .Formulas(7) = "wGSTNo = '" & gCGStin & "'"
                 .Formulas(8) = "wCPane =  '" & gCPAN & "'"
                 If OptCopy(0).Value = True Then
                    .Formulas(9) = "wCopy =  'Customer Copy'"
                 Else
                    .Formulas(9) = "wCopy =  'Office Copy'"
                 End If
                 .ReportFileName = gReportPath & "rptIntDbNtGSTPlain.rpt"
              End If
              .Action = 1
              .PageZoom (120)
...
'    End If
    ...
    If i > 0 Then
       ...
       Loop
    End If
    CriteriaStr = " VType = '" & frmTypeStr & "' and  VnoList <> 0  and Interest > 0 and VFirm in (" & BillsInFirm & ") and VNoListVYear=" & gCYear
    CriteriaStrForGenId = " VType = '" & frmTypeStr & "' and  VnoList <> 0 and VFirm in (" & BillsInFirm & ") and VNoListVYear=" & gCYear
...
If gCIsDepotFirm = 1 Then
   MsgBox ("It Is Depot Firm - Int. Db. Note in Main Firm Please ...")
   cmdBtn_Click (14)
   Exit Sub
End If
...
If txt(35) = "" Or GProcIsDateValid(dtpDt(11)) = False Then
...
If gCYear >= 2021 Then   '--- FOr ItHsn,Ithsn2
    X = "select VBillNo,VBillType, VBillDate,VBillAmt,iif(tblMastAccount.AcIsIntDbNtMonthly = 1,' * '+tblMastAccount.AcName,tblMastAccount.AcName),tblMastAccount_Br.AcName, VAmt,tblVoucher.Vdt,LateDays,Interest,TDSOn,LessTDS,DBNtInt,DbNtInBillNo,DbNtMonthly,DueOnDt,tblVoucher.Vno,VCtrNo,Grace,IntRt,CgstRt,SgstRt,IgstRt,JvNoInt,JvNoLessTDS,tblMastItem.ItHsn2,tblVoucher.VBillFirm,tblVoucher.CrDrNoteNo,tblVoucher.VNoListVYear,tblVoucher.Vyear" _
    & " from tblVoucher,tblMastAccount,tblMastAccount tblMastAccount_Br,tblSale,tblSaleSub,tblMastItem where " _
    & " tblVoucher.VType='" & frmTypeStr & "' and VnoList=" & txt(35) & " and tblVoucher.VFirm in (" & BillsInFirm & ") and  tblVoucher.VnolistVYear=" & gCYear & " and VCrAcCode = tblMastAccount.AcCode and  tblSale.SlBroker = tblMastAccount_Br.AcCode  and " _
    ...
    & " tblSaleSub.SlSubItCode = tblMastItem.ItCode "
...
Else
    X = "select VBillNo,VBillType, VBillDate,VBillAmt,iif(tblMastAccount.AcIsIntDbNtMonthly = 1,' * '+tblMastAccount.AcName,tblMastAccount.AcName),tblMastAccount_Br.AcName, VAmt,tblVoucher.Vdt,LateDays,Interest,TDSOn,LessTDS,DBNtInt,DbNtInBillNo,DbNtMonthly,DueOnDt,tblVoucher.Vno,VCtrNo,Grace,IntRt,CgstRt,SgstRt,IgstRt,JvNoInt,JvNoLessTDS,tblMastItem.ItHsn,tblVoucher.VBillFirm,tblVoucher.CrDrNoteNo,tblVoucher.VNoListVYear,tblVoucher.Vyear" _
    & " from tblVoucher,tblMastAccount,tblMastAccount tblMastAccount_Br,tblSale,tblSaleSub,tblMastItem where " _
    & " tblVoucher.VType='" & frmTypeStr & "' and VnoList=" & txt(35) & " and tblVoucher.VFirm in (" & BillsInFirm & ") and  tblVoucher.VnolistVYear=" & gCYear & " and VCrAcCode = tblMastAccount.AcCode and  tblSale.SlBroker = tblMastAccount_Br.AcCode  and " _
    & " VBillVno = tblSale.Vno and VBillType = tblSale.Vtype and VBillVYear = tblSale.Vyear and (case when isnull(tblvoucher.vBillfirm,'0')='0' then tblvoucher.vfirm else tblvoucher.vBillfirm end)=tblsale.vfirm and tblSale.vno = tblSaleSub.Vno and tblSale.vtype = tblSaleSub.Vtype and tblSale.vyear = tblSaleSub.Vyear and tblSale.vfirm = tblSaleSub.Vfirm and  tblsaleSub.SlSubItSrNo = 1 and " _
    & " tblSaleSub.SlSubItCode = tblMastItem.ItCode "
...
With msGrid
    ...
    .FormatString = "<Bill No           |<Tp |<Bill Date    |>Bill Amount    |<Party Name                     |<Broker Name |>Amt. Recd     |<Pmt. Date  |>OD |>Late Int.        |>TDS On Amt.  |>TDS Amt.  |DN|DN in Bill No|||||||Cgst Rt|Sgst Rt|Igst Rt||TDS JV No.|HSN Cd||Debit Nt. No ||"
    .ColWidth(1) = 0
    .ColWidth(2) = 0
    .ColWidth(5) = 0
    .ColWidth(7) = 0
...
Private Sub txtGrid_GotFocus()
    ...
    If msGrid.Col = 9 Then
        OldTdsOn = Val(txtGrid)
    End If
    DoEvents
...
JvNo = 0
JvNoTDs = 0
gCn.Execute "Delete from tblIntSale where VType in ('SI','MI') and VFirm = '" & gCCode & "'  and VYear = " & gCYear & " and  vnolist  = " & txt(35)
...
End If
DoEvents
gCn.Execute "update tblVoucher set Interest = 0, LessTDS = 0 ,DueOnDt =  '', VnoList = 0 ,Grace = 0,IntRt = 0,LateDays = 0,JvNoInt=0 ,JvNoLessTDS = 0,GraceExtra =0,IntFromDate = null,IntParty = 0,TdsRt = 0,TDSOn=0,DBNtInt = null,DbNtMonthly = 0,DbNtInBillNo = null,CrDrNoteNo = null,VBillFirm = null " _
    ...
    Do While i < .rows
        ...
        If CDec(Val(.TextMatrix(i, 9))) > 0 Then
           ...
           gCn.Execute "update tblVoucher set Interest = " & Val(.TextMatrix(i, 9)) & ", LessTDS = " & Val(.TextMatrix(i, 11)) & ",DueOnDt = '" & IIf(IsDate(.TextMatrix(i, 15)), Format(.TextMatrix(i, 15), "yyyy/mm/dd"), "") & "', VnoList = " & Val(txt(35)) & " ,Grace = " & Val(.TextMatrix(i, 18)) & ",IntRt = " & Val(.TextMatrix(i, 19)) & ",LateDays = " & Val(.TextMatrix(i, 8)) & ",  " _
                       ...
                       & " JvNoInt=" & Val(.TextMatrix(i, 23)) & " ,JvNoLessTDS = " & Val(.TextMatrix(i, 24)) & ", CrDrNoteNo='" & .TextMatrix(i, 27) & "', " _
                       ...
                       & "  IntToDate = '" & IIf(IsDate(dtpDt(2)), Format(dtpDt(2), "yyyy/mm/dd"), "") & "',VNoListVYear = " & gCYear & "    where VNo = " & Val(.TextMatrix(i, 16)) & " and  VType = '" & UCase(txt(1)) & "' and  VFirm = '" & .TextMatrix(i, 26) & "'     and VYear = " & Val(.TextMatrix(i, 29)) & "  and VCtrNo = " & Val(.TextMatrix(i, 17)) & " "
            
'X = "select * from tblvoucher where VNo = " & Val(.TextMatrix(i, 16)) & " and  VType = '" & UCase(txt(1)) & "' and  VFirm = '" & .TextMatrix(i, 26) & "'     and VYear = " & Val(txt(3)) & "  and VCtrNo = " & Val(.TextMatrix(i, 17)) & " "
...
Y = "Select * from tblIntSale where VType = 'SI' and  VFirm  = '" & gCCode & "' and VYear=" & gCYear
GProcRstOpen rstSaleInt, Y, "O"
Mon = Month(dtpDt(11))
GProcRstOpen rsDbNt, "Select * from tblIntDbNote where VnoList = " & txt(35) & " and VType = '" & frmTypeStr & "' and  VnoList <> 0  and Interest > 0 and VFirm in (" & BillsInFirm & ") and VYear= " & gCYear & " ", "O"
...
If gCYear >= 2021 Then ''- For ItHsn2,ItHsn
    Y = "select tblVoucher.VNo,tblVoucher.VType,tblVoucher.Vdt,tblVoucher.VYear,tblVoucher.VFirm,VCtrNo, VCrAcCode,VDrAcCode,VAmt,VBillNo,VBillVno,VBillType,VBillDate,VBillVYear," _
        ...
        & "and tblSaleSub.SlSubItCode = tblMastItem.ItCode and VnoList = " & txt(35) & " and DBNtInt = 'Y' and tblVoucher.VType = '" & frmTypeStr & "' and tblVoucher.VnoList <> 0 and tblVoucher.Interest > 0 and tblVoucher.VFirm in (" & BillsInFirm & ")  and tblVoucher.VNoListVYear= " & gCYear & " "
...
'           If RsP.Fields("VCrAcCode") <> wParty And RsP.Fields("ItHsn") <> wHsnCode And RsP.Fields("DbNtInBillNo") <> wDbNtInBillNo Then
           If RsP.Fields("VCrAcCode") <> wParty And RsP.Fields("DbNtInBillNo") <> wDbNtInBillNo Then
              Exit Do
           End If
           wJvNoTmp = Val(RsP.Fields("JvNoInt"))
           wDbNtNoTmp = RsP.Fields("CrDrNoteNo") & vbNullString
           If wJvNoTmp > 0 Then
              wJvNo = wJvNoTmp
           Else
              If RsP.Fields("VBillType") = "SY" Or Format(CDate(txt(11)), "YYYY/MM/dd") < Format("2018/11/06", "YYYY/MM/dd") Then
                 wJvNo = GProcGenerateIdMonthwise("tblIntSale", "Vno", Mon, "Vdt", "Vtype='SI' and VFirm = '" & gCCode & "' and VYear=" & gCYear)
              Else
                 wJvNo = GProcGenerateIdMonthwise("tblIntSale", "Vno", Mon, "Vdt", "Vtype='MI' and VFirm = '" & gCCode & "' and VYear=" & gCYear)
              End If
           End If
           If wDbNtNoTmp <> "" Then
              wDbNtNo = wDbNtNoTmp
           Else
              If RsP.Fields("VBillType") = "SY" Or Format(CDate(txt(11)), "YYYY/MM/dd") < Format("2018/11/06", "YYYY/MM/dd") Then
                 If chkIsGSTDbNt.Value = 1 Then
                    wDbNtNo = GProcGenerateNoWithIBCrDrNt("SI", gCCode)
                 Else
                    wDbNtNo = GProcGenerateIdMonthwise("tblIntSale", "CrDrNoteNo", Mon, "Vdt", "Vtype='SI' and VFirm = '" & gCCode & "' and VYear=" & gCYear)
                 End If
              Else
                 wDbNtNo = ""
              End If
           ...
           If RsP.Fields("DbNtInBillNo") <> wBlNo Or RsP.Fields("DbNtInBillNo") = "" Then
              ...
              wDbNtTotTmp = Round(Val(RsP.Fields("Interest")) + Round((Val(RsP.Fields("Interest")) * Val(RsP.Fields("CgstRt"))) / 100, 2) + Round((Val(RsP.Fields("Interest")) * Val(RsP.Fields("SgstRt"))) / 100, 2) + Round((Val(RsP.Fields("Interest")) * Val(RsP.Fields("IgstRt"))) / 100, 2), 2)
              ...
              gCn.Execute "update tblVoucher set JvNoInt = " & wJvNo & ", CrDrNoteNo= '" & wDbNtNo & "' " _
       ...
       If wIntAmt > 0 Then
            ...
            If wVBillType = "SY" Or Format(CDate(txt(11)), "YYYY/MM/dd") < Format("2018/11/06", "YYYY/MM/dd") Then
               rstSaleInt!Vtype = "SI"  '--Vtype
               wTp = "SI"
            Else
                If wVBillType = "SY" Then
                   rstSaleInt!Vtype = "SI"  '--Vtype
                   wTp = "SI"
                Else
                   rstSaleInt!Vtype = "MI"  '--Vtype
                   wTp = "MI"
                End If
            End If
            ...
            If wVBillType = "SY" Or Format(CDate(txt(11)), "YYYY/MM/dd") < Format("2018/11/06", "YYYY/MM/dd") Then
               If chkIsGSTDbNt.Value = 1 Then
                  rstSaleInt!VCrAcCode = gLatePayIntAcCodeRecGST
               Else
                  rstSaleInt!VCrAcCode = gLatePayIntAcCodeRec
               End If
            Else
                If wVBillType = "SY" Then
                    If chkIsGSTDbNt.Value = 1 Then
                ...
                Else
                    If chkIsGSTDbNt.Value = 1 Then
                       rstSaleInt!VCrAcCode = gLPIntAcCodeWithoutGST
                    Else
                       rstSaleInt!VCrAcCode = gLatePayIntAcCodeRec
                    End If
                End If
            End If
            rstSaleInt!VDrAcCode = wParty
'            wDbNtTot = Round(wIntAmt + wCgstAmt + wSgstAmt + wIgstAmt, 2)
 '           wDbNtTot = GProcMakeRounding(wDbNtTot)
            rstSaleInt!vamt = wDbNtTot
            rstSaleInt!VNar1 = "Late Payment Debited "
            rstSaleInt!VNar2 = "Agt. Bill No : " & wBlNo
'            rstSaleInt!VNar3 = ""
'            rstSaleInt!VNAR4 = ""
            rstSaleInt!Interest = wIntAmt
            ...
            '-- For Log
            If gCYear >= 2023 And gCIsLog = 1 Then
               XX = "Select max(LogNo) from tblIntSale_Log where Vno=" & wJvNo & " and vtype = '" & wTp & "' and Vfirm = '" & gCCode & "' and vyear = " & txt(3) & " and VCtrNo = " & i - 1 & " "
               jj = GProcRstOpen(tmpRst, XX, "R")
               If IIf(IsNull(tmpRst.Fields(0)), 0, tmpRst.Fields(0)) > 0 Then
                  wLogNo = IIf(IsNull(tmpRst.Fields(0)), 0, tmpRst.Fields(0)) + 1
                  AMD = "M"
               ...
               gCn.Execute "Insert into tblIntSale_Log select tblIntSale.*," & wLogNo & " as LogNo,'" & AMD & "' as LogTp ,'" & gUserName & "' as UserName ,'" & Format(Now, "YYYY/MM/DD hh:mm:ss") & "' as LogDate from tblIntSale where Vno=" & wJvNo & " and vtype = '" & wTp & "' and Vfirm = '" & gCCode & "' and vyear = " & txt(3) & " and VCtrNo = " & i - 1 & " "
               tmpRst.Close
            End If
        End If
        '--- TDS Db Nt.
        If wTdsAmt > 0 Then
            If j = 1 Then
               If Val(txt(34)) > 0 Then
                  wJvNoTDS = txt(34)
               Else
                   wJvNoTDS = GProcGenerateIdMonthwise("TblVoucher", "Vno", Mon, "Vdt", "Vtype='JV' and VFirm = '" & gCCode & "' and VYear=" & gCYear)
               End If
            End If
            gCn.BeginTrans
            rstbl.AddNew
...
'            rstbl!VNar1 = "T D S @ " + CStr(gTDSRate) + " % On Int. Of Rs. " + CStr(wTdsOn) + " Debited"
            rstbl!VNar1 = "T D S @ " + txt(42) + " % On Int. Of Rs. " + CStr(wTdsOn) + " Debited"
            ...
            '-- For Log
            If gCYear >= 2023 And gCIsLog = 1 Then
               XX = "Select max(LogNo) from tblVoucher_Log where Vno=" & wJvNoTDS & " and vtype = 'JV' and Vfirm = '" & gCCode & "' and vyear = " & txt(3) & " and VCtrNo = " & j - 1 & " "
               jj = GProcRstOpen(tmpRst, XX, "R")
               If IIf(IsNull(tmpRst.Fields(0)), 0, tmpRst.Fields(0)) > 0 Then
                  wLogNo = IIf(IsNull(tmpRst.Fields(0)), 0, tmpRst.Fields(0)) + 1
                  AMD = "M"
               Else
                  ...
                  AMD = "A"
               End If
               gCn.Execute "Insert into tblVoucher_Log select tblVoucher.*," & wLogNo & " as LogNo,'" & AMD & "' as LogTp ,'" & gUserName & "' as UserName ,'" & Format(Now, "YYYY/MM/DD hh:mm:ss") & "' as LogDate from tblVoucher where Vno=" & wJvNoTDS & " and vtype = 'JV' and Vfirm = '" & gCCode & "' and vyear = " & txt(3) & " and VCtrNo = " & j - 1 & ""
               tmpRst.Close
            End If
       End If
'''       RsP.MoveNext
...
If (GProcRstOpen(RsP, Y, "O") > 0) Then
    wParty = 0
    wJvNo = GProcGenerateIdMonthwise("tblIntDbNote", "Vno", Mon, "Vdt", "Vtype='SN' and VFirm = '" & gCCode & "' and VYear=" & gCYear)
    i = 1
    j = 1
    RsP.MoveFirst
    Do While Not RsP.EOF
       ...
       '--- Interest Db Nt.
        If wIntAmt > 0 Then
            ...
            rsDbNt!Vtype = "SN"  '--Vtype
            rsDbNt!Vdt = CDate(txt(11)) '--Vdt(2)
            rsDbNt!VYear = txt(3) '--VYear
            rsDbNt!VFirm = gCCode '--VFirm
            rsDbNt!VCtrNo = i '--Sr No
            i = i + 1
            If chkIsGSTDbNt.Value = 1 Then
               rsDbNt!VCrAcCode = gLatePayIntAcCodeRecGST
            Else
            ...
            rsDbNt!VBillFirm = wVBillFirm
            rsDbNt!CgstAmt = wCgstAmt
            rsDbNt!SgstAmt = wSgstAmt
            rsDbNt!IgstAmt = wIgstAmt
            rsDbNt.Update
...
If gCYear >= 2021 Then '--For ItHsn2,ItHsn
    X = "Select a.VBillNo,d.Vtype,a.VBillDate,a.VBillVYear,d.VFirm,d.SlBillAmt,a.VAmt,a.Vdt,b.AcOurCode,b.Acname,dateadd(dd,d.DueDays+d.SlCrDays+iif(b.AcDueDays>0,b.AcDueDays," & Val(txt(37)) & ") ,d.Vdt) , " _
                       ...
                       & " and d.SlAcDrCode = b.AcCode and d.SlBroker = c.AcCode and a.Vtype = '" & frmTypeStr & "' and d.vno = e.vno and d.vtype = e.vtype and d.vyear = e.vyear and d.vfirm = e.vfirm and  e.SlSubItSrNo = 1 and e.SlSubItCode = f.ItCode" _
   ...
   Do While Not RsTmp.EOF
      With msGrid
           ...
           If Val(RsTmp.Fields(15)) > Val(txt(39)) Then
                x1 = "select * from tblvoucher where vtype = 'BP' and  "
                j = .rows - 1
                ...
                Else
                   IntRt = Val(txt(38))
                End If
                Lint = (((CDbl(RsTmp.Fields(6)) * IntRt) / 100) / 365) * (Val(RsTmp.Fields(15)))
                ...
                If Format(dtpDt(11), "YYYY/MM/dd") <= Format("2018/04/01", "YYYY/MM/dd") Then
                   ...
                   If Left(wgTin, 2) = "27" Or Left(wgTin, 1) = "U" Or Trim(wgTin) = "" Then
                      .TextMatrix(j, 20) = Format(RsTmp.Fields("SltaxRate") / 2, FStr)             '20 - Cgst rt
                      .TextMatrix(j, 21) = Format(RsTmp.Fields("SltaxRate") / 2, FStr)             '21 - Sgst rt
                      .TextMatrix(j, 22) = 0
                   Else
                      .TextMatrix(j, 20) = 0                                                       '20 - Cgst rt
                      .TextMatrix(j, 21) = 0                                                       '21 - Sgst rt
                      .TextMatrix(j, 22) = Format(RsTmp.Fields("SltaxRate"), FStr)
                   End If
                ElseIf Format(dtpDt(11), "YYYY/MM/dd") >= Format("2018/11/06", "YYYY/MM/dd") Then
                    If RsTmp.Fields(1) = "SY" Then
                       .TextMatrix(j, 20) = Format(RsTmp.Fields("SltaxRate"), FStr)                     '20 - Cgst rt
                       .TextMatrix(j, 21) = Format(RsTmp.Fields("SltaxRate2"), FStr)                    '20 - Sgst rt
                       .TextMatrix(j, 22) = Format(RsTmp.Fields("SltaxRate3"), FStr)                    '20 - Igst rt
                    Else
                ...
                Else
                    If RsTmp.Fields(1) = "SM" Then
                        ...
                        If RsTmp.Fields("ittype") = 0 Then
                           If Left(wgTin, 2) = "27" Or Left(wgTin, 1) = "U" Or Trim(wgTin) = "" Then
                              .TextMatrix(j, 20) = Format(CotCrt, FStr)                                      '20 - Cgst rt
                              .TextMatrix(j, 21) = Format(CotSrt, FStr)                                      '21 - Sgst rt
                              .TextMatrix(j, 22) = 0
                           Else
                              .TextMatrix(j, 20) = 0
                              .TextMatrix(j, 21) = 0
                              .TextMatrix(j, 22) = Format(CotIrt, FStr)
                           End If
                        Else
                           If Left(wgTin, 2) = "27" Or Left(wgTin, 1) = "U" Or Trim(wgTin) = "" Then
                              .TextMatrix(j, 20) = Format(PolCrt, FStr)                                      '20 - Cgst rt
                              .TextMatrix(j, 21) = Format(PolSrt, FStr)                                      '21 - Sgst rt
                              .TextMatrix(j, 22) = 0
                              If Format(RsTmp.Fields(2), "YYYY/MM/dd") <= Format("2017/10/12", "yyyy/mm/dd") Then
                                .TextMatrix(j, 20) = Format(9, FStr)                                      '20 - Cgst rt
                                .TextMatrix(j, 21) = Format(9, FStr)                                      '21 - Sgst rt
                                .TextMatrix(j, 22) = 0
                              End If
                           Else
                              .TextMatrix(j, 20) = 0
                              .TextMatrix(j, 21) = 0
                              .TextMatrix(j, 22) = Format(PolIrt, FStr)                                      '22 - Igst Rt
                              If Format(RsTmp.Fields(2), "YYYY/MM/dd") <= Format("2017/10/12", "YYYY/MM/dd") Then
                                .TextMatrix(j, 20) = 0
                                .TextMatrix(j, 21) = 0
                                .TextMatrix(j, 22) = Format(18, FStr)
                              End If
...
If OldTdsOn <> Val(msGrid.TextMatrix(msGrid.Row, 9)) And OldTdsOn <> 0 And FormAction = vbDataActionAddNew Then
