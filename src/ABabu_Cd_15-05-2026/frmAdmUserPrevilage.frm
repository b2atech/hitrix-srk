...
Case 3 'Last
    ...
    ReadFields
Case 4 'Find
    gClsSearch.SearchMultiField tblName, "UserName", Array("User Name"), Array(txt(0).Width), " srno=1", "", "UserName", txt(0).Left + Me.Left, txt(0).Top + Me.Top + 650, False, gCn
...
If Index = 0 Then '-- User Name
    gClsSearch.SearchMultiField "tblMastUser", "UserName", Array("User Name"), Array(txt(Index).Width), " isAdminUser=0 ", Chr(KeyAscii), "UserName", LeftPos, TopPos, True, gCn
...
'        End If
    ...
    If FormAction = vbDataActionAddNew Then
        ...
        i = GProcRstOpen(rstAddCheck, X, "R", gCn)
        If i > 0 Then
...
Do While Not rstbl.EOF
    ...
    If IsNodeExist(MNKey) = True Then
        ...
        If rstbl.Fields("Add_R") = 1 Then
            MNKey = MNname & "-A" & MNIndex
            TreeView1.Nodes(MNKey).Checked = True
        End If
        If rstbl.Fields("Modify_R") = 1 Then
            MNKey = MNname & "-M" & MNIndex
            TreeView1.Nodes(MNKey).Checked = True
        End If
        If rstbl.Fields("delete_R") = 1 Then
            MNKey = MNname & "-D" & MNIndex
            TreeView1.Nodes(MNKey).Checked = True
        End If
        If rstbl.Fields("View_R") = 1 Then
...
'============= Previlage for menu "mnutl", "mnutrn", "mnurpt", "mnumst" Only
    Case "mnuutl", "mnutrn", "mnurpt", "mnumst", "mnutrnGst"
