...
Begin VB.Form frmMastFirmwiseIntRate 
   BackColor       =   &H00FBF2E1&
   BorderStyle     =   0  'None
   ...
   FillColor       =   &H000040C0&
   FillStyle       =   0  'Solid
   Icon            =   "frmMastFirmwiseIntRate.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin Crystal.CrystalReport cryReport 
      Bindings        =   "frmMastFirmwiseIntRate.frx":058A
      Left            =   4110
      Top             =   0
      _ExtentX        =   741
      _ExtentY        =   741
   ...
   Begin VB.Label Label6 
      BackColor       =   &H00EFB17A&
      Caption         =   "     Firmwise Interest Rate  Master"
      BeginProperty Font 
         Name            =   "Times New Roman"
         Size            =   15.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmMastFirmwiseIntRate"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Const tblName As String = "tblMastFirmwiseIntRate"
Const IdField As String = "Vno"
Const OrderField As String = "Firm"
...
Case 4 'Find ' QryMastAccount
    gClsSearch.SearchMultiField "tblMastFirmwiseIntRate,TblMastAccount", "AcName,Vno", Array("A/c. Name", "Vno"), Array(6000, 1500), " tblMastFirmwiseIntRate.PartyCd=TblMastAccount.AcCode and CtrNo = 1 ", "", "AcName,Vno", 0, 0
...
Case 6 'Add
    ...
    If FirstTimeFlag = True Then
        FirstTimeFlag = False
    End If
    txt(0) = GProcGenerateId("tblMastFirmwiseIntRate", "Vno", "")
    SetGrid
Case 7 'Modify
...
''--- Voucher Sub
gCn.Execute "delete from tblMastFirmwiseIntRate where vno=" & txt(0) & " "
End Sub
...
'--- Save Grid Records
'--- TblVouSub
X = "select * from tblMastFirmwiseIntRate where  vno=" & txt(0) & " "
...
With msGrid
    ...
    Do While i < .rows
            ...
            rstSub!PartyCd = MillCode
            rstSub!CtrNo = i '--Sr No
            rstSub!VFirm = GProcGetColumnValue("tblMastCompany", "CName", .TextMatrix(i, 1), "S", "CCode", "N")
            rstSub!IntRate = Val(.TextMatrix(i, 2))
            rstSub.Update
            i = i + 1
     Loop
 End With
...
X = "select CtrNo,CName,IntRate" _
& "  from tblMastFirmwiseIntRate,tblMastCompany where " _
& "  tblMastFirmwiseIntRate.vno=" & txt(0) & "  " _
& " and tblMastFirmwiseIntRate.VFirm = tblMastCompany.CCode   order by CtrNo "
GProcRstOpen rstblSub, X, "R"
With msGrid
   .FormatString = ">Sr|<Firm Name                                                     |>Interest Rate"
    If FormAction = vbDataActionAddNew Then
        .rows = 2
        GridAddNew
    Else
...
Select Case Index
    Case 1  '-- Mill  Alias Account
        gClsSearch.SearchMultiField "tblMastaccount", "AcName,AcCity,AcOurCode", Array("Sales A/c Name", "City", ""), Array(7000, 2000, 0), "", Chr(KeyAscii), "AcName", LeftPos, TopPos, True
        KeyAscii = 0
        txtCode(1) = ""
        If gClsSearch.SearchMultiRetCol(0) <> "" Then
...
With msGrid
    If KeyAscii <> 13 And KeyAscii <> 27 And KeyAscii <> 25 Then
        ...
        Select Case .Col
            Case 1 '--Booking
                    gClsSearch.SearchMultiField "tblMastCompany", "CName,CCode", Array("Firm Name", "Code"), Array(5000, 0), "", Chr(KeyAscii), "CName", LeftPos, TopPos, True
                    txtGrid = ""
                    KeyAscii = 0
            ...
            Select Case i
                Case 1 '-- sale type
                    If .TextMatrix(RowIndex, 1) = "" Then
                        MsgBox "Check Firm Name", vbCritical + vbOKOnly, Me.Caption
                        .Col = i
                        msGrid.SetFocus
                        Exit Function
                    End If
...
'    MillCtrlEd
    ...
    '---- Mill Account
    If txttemp(1) = "" Then
        MsgBox "Check  A/c Name", vbInformation + vbOKOnly, Me.Caption
        txttemp(1).SetFocus
        Exit Function
    End If
    '----Duplicate Records
    '----Addition Case
    If FormAction = vbDataActionAddNew Then
        X = "Select vno From " & tblName & " Where vno=" & CLng(txt(0)) & "  "
    ...
    End If
    If MilAcCode > 1 Then
        X = "Select vno From " & tblName & " Where vno<>" & Val(txt(0)) & " and PartyCd = " & MilAcCode & "  "
        i = GProcRstOpen(rstAddCheck, X, "R")
        If i > 0 Then
...
'-- Mill Code
txt(3) = GProcGetColumnValue("tblMastAccount", "AcName", txttemp(1), "S", "AcCode", "N")
End Sub
Private Sub FillTempFromTxt()
Dim X As String
'-- Mill Code
txttemp(1) = GProcGetColumnValue("tblMastAccount", "AcCode", txt(3), "N", "AcName", "S")
txtCode(1) = GProcGetColumnValue("tblMastAccount", "AcCode", txt(3), "N", "AcOurCode", "S")
FillRelatedRecord
End Sub
